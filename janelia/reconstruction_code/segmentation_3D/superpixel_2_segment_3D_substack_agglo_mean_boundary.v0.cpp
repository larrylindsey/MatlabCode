// Superpixel to segment 3D agglomerative mean boundary. Combine
// substack segment.
//
// Shiv N. Vitaladevuni
...
int main(int argc, char * argv[]){
  ...
  printf("\nSTART: superpixel_2_segment_3D_substack_agglo_mean_boundary_b\n");
  ...
  for(unsigned int substack_id=0; substack_id<n_substack; substack_id++){
    ...
    printf("Reading in the segment label mapping\n");
    {
      ...
      }
      {
        ...
        for(i=0; i<max_superpixel_labels[substack_id]; i++)
          label_mappings[substack_id][i] = label_mapping[i];
      }
      
      {
        unsigned int max_label_mapping=0;
        for(int i=0; i<max_superpixel_labels[substack_id]; i++){
      ...
      for(i=0; i<stack_n_pixel; i++, s++, b++){
        ...
        if(is_boundary){
          ...
          for(n1=0; n1<n_neighbor; n1++){
            s1 = label_mappings[substack_id][
              superpixel_label_map[neighbors[n1]]];
            if(s1==0)
              continue;
          ...
          segment_sets.add_new_set_inc(label_mappings[substack_id][*s]);
        }
      }
  ...
  printf("\nSTOP: superpixel_2_segment_3D_substack_agglo_mean_boundary_b\n");
  return 0;
}

function em_reconstruct_on_cluster_linkage_evaluation(config_param, ...
  train_case_ids, test_case_ids, varargin)
...
job_dir = ['cluster_jobs/', num2str(test_case_ids(1)), '_', ...
  num2str(test_case_ids(end)), '/'];
job_name_prefix = '';
...
is_verbose = true;
if(is_verbose)
  fprintf('START: em_reconstruct_on_cluster_linkage_evaluation\n');
end
...
% Compute linkage graphs and evaluate them
job_name_prefix_full_3 = [job_name_prefix, 'el'];
em_reconstruct_on_cluster_synch(config_param, '750 920.1', ...
  get_cell_of_sections(test_case_ids(1:end-1), 2), job_dir, ...
  job_name_prefix_full_3, 'prerequisite_job', ['end_', job_name_prefix_full_2], ...
  'is_split_by_tiles', true);
    
% Compile evaluation results
job_name_prefix_full_4 = [job_name_prefix, 'elc'];
em_reconstruct_on_cluster_synch(config_param, '920.5', ...
  {test_case_ids}, job_dir, job_name_prefix_full_4, ...
  'prerequisite_job', ['end_', job_name_prefix_full_3]);

fout = fopen([job_dir, job_name_prefix, 'eval_link.sh'], 'wt');
fprintf(fout, './%sltf.sh\n./%sltc.sh\n./%sel.sh\n./%selc.sh\n', ...
  job_name_prefix, job_name_prefix, job_name_prefix, job_name_prefix);
fclose(fout);
system(['chmod u+x ', job_dir, job_name_prefix, 'eval_link.sh']);
fprintf('To submit jobs, run %s%seval_link.sh\n', job_dir, job_name_prefix);

if(is_verbose)
  fprintf('STOP: em_reconstruct_on_cluster_linkage_evaluation\n');
end

function superpixel_2_segment_BEL_ladder(config)
% superpixel_2_segment_BEL_ladder(config)
% Perform 2D segmentation based on superpixels using Boosted Edge Learning
% (BEL) boundary detection with ladder 
%
% Shiv N. Vitaladevuni
...
for case_id = stack_config.case_ids
  ...
  for tile_id = 1:length(images)
    ...
    superpixel_seg = load2([superpixel_dir, image_prefix, ...
        superpixel_suffixes{1}, '.mat'],  'label_map');
    ...
    for s = 1:length(seg_config.f_thresholds)
      for t = 1:length(seg_config.area_thresholds)
        ...
        save_file_name_suffix = ['.BEL_l_sp_T', num2str(f_threshold), '_L', num2str(area_threshold), ...
          seg_config.superpixel_suffix, '.', version_name];

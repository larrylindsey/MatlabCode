...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[]){
...
#endif
    {
      ...
      for(lpq_it=label_pair_list.begin(); lpq_it!=label_pair_list.end();
        ...
        while(q_it!=merge_q.end()){
          m += 0.00002*((double)(rand()%1000)); // epsilon increments
          q_it = merge_q.find(m);
        }
        {
...

  {
    ...
    int p, prev_p, next_p;
    Label_Pair lp_li, lp_il, lp_ir, lp_ri, lp_lr, lp_rl, lp_10;
    q_it = merge_q.begin();
    m = (*q_it).first;
    tie(s0,s1) = id_2_label_pair((*q_it).second);
    is_valid_merge_q_pos[(*q_it).second]=0;
#ifdef __DEBUG__
    fprintf(fout_db, "merge_q.erase(%lu)", (*q_it).second);
    fflush(fout_db);
#endif //__DEBUG__
    merge_q.erase(q_it);
...
#ifdef __DEBUG__
      fprintf(fout_db, "merge_q.erase(%lu)", lp_10);
      fflush(fout_db);
#endif //__DEBUG__
    ...
    while(f_threshold_id<n_f_threshold){
      ...
      while(q_it!=merge_q.end() && m<f_threshold){ // merge segments s0 and s1
        ...
        if(superpixel_sets.get_adam(s0)==superpixel_sets.get_adam(s1)){
          q_it = merge_q.begin();
          if(q_it==merge_q.end())
            break;
          m = (*q_it).first;
        ...
        while(p>0){ // add the neighbors of sl to sr
...
#ifdef __DEBUG__
            fprintf(fout_db, "neighbor marked for deletion ...\n");
            fflush(fout_db);
#endif //__DEBUG__

            if(prev_p<=0)
              adjacency_list_pointer[sl] = adjacency_lists[p].next_ptr;
            else
              adjacency_lists[prev_p].next_ptr = adjacency_lists[p].next_ptr;
            p = next_p;
            
#ifdef __DEBUG__
            fprintf(fout_db, "deleted it and skipping.\n");
            fflush(fout_db);
#endif //__DEBUG__
          ...
          if(label_adjacency[lp_ir]==0){ // si is not a neighbor of sr
...
#ifdef __DEBUG__
              fprintf(fout_db, "transferring merge q pos %lu\n", lp_il);
              fflush(fout_db);
#endif //__DEBUG__
...
#ifdef __DEBUG__
            fprintf(fout_db, "deleting the boundaries from merge q...\n");
            fflush(fout_db);
#endif //__DEBUG__
            if(is_valid_merge_q_pos[lp_il]==1){
#ifdef __DEBUG__
              fprintf(fout_db, "merge_q.erase(%lu)", lp_il);
              fflush(fout_db);
#endif //__DEBUG__
            ...
            if(is_valid_merge_q_pos[lp_li]==1){
#ifdef __DEBUG__
              fprintf(fout_db, "merge_q.erase(%lu)", lp_li);
              fflush(fout_db);
#endif //__DEBUG__
...
#ifdef __DEBUG__
              fprintf(fout_db, "merge_q.erase(%lu)", lp_ir);
              fflush(fout_db);
#endif //__DEBUG__
...
#ifdef __DEBUG__
              fprintf(fout_db, "merge_q.erase(%lu)", lp_ri);
              fflush(fout_db);
#endif //__DEBUG__
...
#ifdef __DEBUG__
              fprintf(fout_db, "added merge q pos %lu\n", lp_ir);
              fflush(fout_db);
#endif //__DEBUG__
...
#ifdef __DEBUG__
            fprintf(fout_db, "deleting sr from sl's list ... ");
            fflush(fout_db);
#endif //__DEBUG__
...
#ifdef __DEBUG__
            fprintf(fout_db, "done with merge-pair.\nGetting next pair ...\n");
            fflush(fout_db);
#endif //__DEBUG__
        q_it = merge_q.begin();
        if(q_it==merge_q.end())
        ...
        is_valid_merge_q_pos[(*q_it).second]=0;
#ifdef __DEBUG__
        fprintf(fout_db, "s0:%u, s1:%u, merge_q.erase(%lu)",
                s0, s1, (*q_it).second);
        fflush(fout_db);
#endif //__DEBUG__
        merge_q.erase(q_it);
        lp_10 = label_pair_2_id(s1, s0);
#ifdef __DEBUG__
        fprintf(fout_db, "deleting reciprocal pair\n");
        fflush(fout_db);
#endif //__DEBUG__
        if(is_valid_merge_q_pos[lp_10]==1){
#ifdef __DEBUG__

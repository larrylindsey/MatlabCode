function superpixel_2_seg_grayscale_agglo_boundary_vs_interior_FisherLD(config)
% superpixel_2_seg_grayscale_agglo_boundary_vs_interior_FisherLD(config)
...
if(strcmp(seg_config.method, 'grayscale_AglBIFL')==0)
...
for case_id = stack_config.case_ids
  ...
  for tile_id = 1:length(images)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%
    % which pixels should be included in a segment's statistics
    exclusion_mask = zeros(size(image));

    if((isfield(seg_config,'use_mitochondria') && seg_config.use_mitochondria) || ...
        (isfield(seg_config,'exclude_mitochondria_stat') && seg_config.exclude_mitochondria_stat))
    ...
    %%%%%%%%%%%%%%%%%%%%%%%
    fprintf('computing agglomerative clustering: boundary vs interior FLD .. ');
    ...
    for superpixel_id = 1:length(superpixel_suffixes)
      ...
      for t = 1:length(seg_config.f_threshold_seq)
        ...
        save_file_name_suffix = ['.gs_abif_sp_T', num2str(f_threshold), ...
        ...
        [label_map, superpixel_to_seg_label] = ...
          ...
          boundary, f_threshold, length_threshold, seg_config.max_area_threshold, ...
          exclusion_mask_current); %#ok<NASGU>

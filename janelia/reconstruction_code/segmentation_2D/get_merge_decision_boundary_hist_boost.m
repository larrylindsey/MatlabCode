function [label_map_new, sp_to_seg_map] = ...
  get_merge_decision_boundary_hist_boost(label_map, boundary_map, ...
  boundary_sets, merge_criterion_param, image, config) %#ok<INUSL>
...
model = load('/groups/chklovskii/home/vitaladevunis/research/em_reconstruction_pipeline/reconstructions/medulla.HPF.Leginon.3500x.zhiyuan.fall2008/linkage_3D/link_model.boost_lp_p3.40.2.overlap_hist_boundary_907234808658_40.mat');
weak_learner = tree_node_w(model.model_config.tree_depth_insection); %#ok<NASGU>
...
while(~no_change)
  ...
  for i = 1:length(interior_map)
    ...
    weak_learner = tree_node_w(model.model_config.tree_depth_insection); %#ok<NASGU>
    confidence_boundary = Classify(model.classifier_insection.GLearners, ...
      model.classifier_insection.GWeights, features');
    
    to_merge_sets = [to_merge_sets; boundary_hist(...
      confidence_boundary>merge_criterion_param.threshold, [1 2])]; %#ok<AGROW>
  end

function segment_2_segment_main(config)
...
if(seg_config.is_verbose)
  fprintf('START: segment_2_segment_main\n');
end
...
for case_id = stack_config.case_ids
  ...
  for tile_id = 1:length(images)
    ...
    [height, width, n_color] = size(image); %#ok<ASGLU>
    if(n_color==3)
      image = image(:,:,1);
    end;
    ...
    for suffix_id = 1:length(superpixel_suffixes)
      ...
      switch(seg_config.method)
        case 'grayscale_ladder'
          segment_2_segment_sub_ladder(superpixel_seg, boundary, image, ...
            seg_config, version_name, save_prefix, superpixel_suffix);
        case {'grayscale_AglBIFL', 'grayscale_AglMeanB', 'grayscale_AglMedianB'}
          segment_2_seg_sub_agglo_AMB_ABIFL_AMdB(...
            superpixel_seg, boundary, image, seg_config, version_name, ...
            save_prefix, superpixel_suffix, exclusion_mask);
        case 'prune_classify'
          segment_2_segment_sub_prune_classify(superpixel_seg, boundary, image, ...
            seg_config, version_name, save_prefix, superpixel_suffix, config, image_prefix);
        case 'shrink_levelset'
          segment_2_segment_sub_shrink_levelset(superpixel_seg, boundary, image, ...
            seg_config, version_name, save_prefix, superpixel_suffix);
        otherwise
...
if(seg_config.is_verbose)
  fprintf('STOP: segment_2_segment_main\n');
end
return

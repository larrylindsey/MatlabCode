...
/*
    mexPrintf("Usage seg_boundary_sets = get_segment_boundary_band_sets(segment_label_map, boundary_map);\n");
    ...
    mexPrintf("\t4. width of the band around boundaries\n");
    mexPrintf("output:\n");
...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[]){
  ...
  double * boundary_input = mxGetPr(prhs[2]);
  
  double * boundary_mask = (double *) new double[n_pixel];
  double * b_m = boundary_mask, * b_i = boundary_input;
  for(int i=0; i<n_pixel; i++, b_m++, b_i++)
    *b_m = *b_i;
  
  int band_width = (int) * mxGetPr(prhs[3]);
  // set 0's along a margin around the image to take care boundary cases
  double * b_m_1=boundary_mask, * b_m_2 = boundary_mask + n_pixel -1;
  for(int x=0; x<band_width; x++)
    for(int y=0; y<height; y++, b_m_1++, b_m_2--){
      *b_m_1 = 0;
      *b_m_2 = 0;
    }
  b_m_1 = boundary_mask;
  b_m_2 = boundary_mask + height - band_width;
  for(int x=0; x<width; x++, b_m_1+=height-band_width, b_m_2+=height-band_width)
    for(int y=0; y<band_width; y++, b_m_1++, b_m_2++){
      *b_m_1 = 0;
      *b_m_2 = 0;
    }
  ...
  // Collect elements
  double * f_map = watershed_field, * f_map_1;
  int n_boundary_samples = n_boundary_elements*4*(2*band_width+1)*(2*band_width+1);
  Boundary * boundary_list = (Boundary *) new Boundary[n_boundary_samples];
  p_ws_l = padded_watershed_label_map+height+2+1;
  b_m = boundary_mask;
  int j=0;
  int height_padded = height+2;
  int label_0, label_1;
  int f_map_offset = height-2*band_width-1;
  for(int x=0; x<width; x++, p_ws_l+=2){
    for(int y=0; y<height; y++, f_map++, p_ws_l++, b_m++){
      if((*p_ws_l)==0 && (*b_m)==1){
        label_0=(int)*(p_ws_l-1); label_1=(int)*(p_ws_l+1);
        f_map_1 = f_map - band_width - height*band_width;
        for(int x1 = x-band_width; x1<=x+band_width; x1++, f_map_1+=f_map_offset)
          for(int y1 = y-band_width; y1<=y+band_width; y1++, f_map_1++){
            if(label_0<label_1){
            ...
            boundary_list[j].f=*f_map_1;
            boundary_list[j].x = x1;
            boundary_list[j].y = y1;
            j++;
          }
  ...
  std::sort(boundary_list, boundary_list+n_boundary_samples-1);
  ...
  for(int i=0; i<n_boundary_samples; i++, b_l++){

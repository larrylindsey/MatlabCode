...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[]){
  ...
  for(int x=0; x<width; x++, p_ws_l+=2){
	...
	if(label_0>0 && label_1>0 && label_0!=label_1){
	  if(label_0<label_1){
  ...
  int n_boundary_points = j;
  
  // Get the list of boundary elements to be involved in the watershed
  std::sort(boundary_list, boundary_list+n_boundary_points);
  
  int * boundary_lengths = (int *) new int[n_boundary_points];
  ...
  for(int i=0; i<n_boundary_points; i++, b_l++){
  ...
  const char * field_names[] = {"segment_label_0", "segment_label_1", 
				"boundary_set"};
  mxArray * seg_boundary_sets = mxCreateStructArray(1, temp_size, 
						    3, field_names);
  for(int i=0; i<n_adj_seg_pair; i++){
    mxSetFieldByNumber(seg_boundary_sets, i, 0, 
		       mxCreateDoubleMatrix(1, 1, mxREAL));
    mxSetFieldByNumber(seg_boundary_sets, i, 1, 
		       mxCreateDoubleMatrix(1, 1, mxREAL));
    mxSetFieldByNumber(seg_boundary_sets, i, 2, 
		       mxCreateDoubleMatrix(3, boundary_lengths[i], mxREAL));
  }
  ...
  for(int i=0; i<n_boundary_points; i++, b_l++){
    if(b_l->label_1!=prev_label_1 || b_l->label_0!=prev_label_0){
      ...
      boundary_set[0] = b_l->x+1;
      boundary_set[1] = b_l->y+1;
      ...
      *(mxGetPr(mxGetFieldByNumber(seg_boundary_sets, pair_id, 1))) = b_l->label_1;
      ...
      if(b_l->y!=prev_y || b_l->x!=prev_x){
        boundary_set[3*boundary_length  ] = b_l->x+1;
        boundary_set[3*boundary_length+1] = b_l->y+1;

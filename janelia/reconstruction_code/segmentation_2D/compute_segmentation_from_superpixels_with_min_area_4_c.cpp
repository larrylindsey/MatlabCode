  ...
  int n_old_label = n_label;
	...
	for(int x=0; x<width; x++, p_ws_l+=2){
		for(int y=0; y<height; y++, f_map++, p_ws_l++){
			if((*p_ws_l)==0){
				boundary_list[j+1].f=*f_map; 	boundary_list[j+1].label_0=(int)*(p_ws_l-1); 						boundary_list[j+1].label_1=(int)*(p_ws_l+1);
        boundary_list[j+1].x = x; boundary_list[j+1].y = y;
				
        boundary_list[j+4].f=*f_map; 	boundary_list[j+4].label_0=(int)*(p_ws_l-height_padded); 		boundary_list[j+4].label_1=(int)*(p_ws_l+height_padded);
	...
	std::sort(boundary_list, boundary_list+n_boundary_elements*24-1);
	...
	for(int i=0; i<n_boundary_elements*24; i++, b_l++){

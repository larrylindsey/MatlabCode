function get_segment_2D_normalized_rand_score(config)
...
for id = 1:length(stack_config.case_ids)
  ...
  if(isfield(evaluate_config, 'seg_suffix'))
    ...
    rand_score = evaluate_boundary(segmentation_map.label_map, ...
  ...
  title('Segmentation Evaluation Normalized Rand');
  if(isfield(evaluate_config, 'seg_suffix'))
...
if(exist(get_storage_file_name(...
    [seg_dir, evaluate_config.dir, 'seg_norm_rand', suffix, '.fig']), 'file')==2)
...
if(isempty(write_flag) || strcmp(write_flag, 'y')==1)
  saveas(figure_id_plot, ...
    ...
    evaluate_config.master_save_prefix, 'seg_norm_rand', ...
    suffix, '.fig']), 'fig');
  save2([seg_dir, evaluate_config.dir, evaluate_config.master_save_prefix, ...
    'seg_norm_rand', suffix, '.mat'], 'rand_score_individual', ...
...
rand_score = get_normalized_rand_score(uint32(seg_gt), ...

...
for j = 1:numel(presentations),
  ...
  ignore = mkdir2(fullfile('pb',pres));
end

ignore = mkdir2('pb/random');
for iid = iids,
...
for sigma = [1 2 4 8 16],
  ignore = mkdir2(sprintf('pb/gm_%d',sigma));
  for iid = iids,
    im = rgb2gray(imgRead(iid));
...
for sigma = [1 2 4 8 16],
  ignore = mkdir2(sprintf('pb/2mm_%d',sigma));
  for iid = iids,
    im = rgb2gray(imgRead(iid));
...
for sigma = [1 2 4],
  ignore = mkdir2(sprintf('pb/gm_%d_%d',sigma,2*sigma));
  for iid = iids,
    im = rgb2gray(imgRead(iid));
...
for sigma = [1 2],
  ignore = mkdir2(sprintf('pb/2mm_%d_%d',sigma,2*sigma));
  for iid = iids,
    im = rgb2gray(imgRead(iid));
...
for sigma = [2 4],
  ignore = mkdir2(sprintf('pb/canny_%d',sigma));
  for iid = iids,
    im = rgb2gray(imgRead(iid));

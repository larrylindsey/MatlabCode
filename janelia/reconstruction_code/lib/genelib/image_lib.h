...
#define GREY32   4   // 4-byte unsigned integer image or stack 
...
#define IMAGE_PIXEL_u32(img,x,y,c) \
...
{ if (image->kind == GREY)
    ...
    return (*IMAGE_PIXEL_u32(image,x,y,c));
}
...
#define STACK_PIXEL_u32(img,x,y,z,c) \
      ((uint32 *) ((img)->array +   \
           ((((z)*(img)->height + (y))*(img)->width + (x))*(img)->kind + (c))))
...
{ if (stack->kind == GREY)
    ...
    return (*STACK_PIXEL_u32(stack,x,y,z,c));
}
...
{ if (stack->kind == GREY)
    ...
    *STACK_PIXEL_u32(stack,x,y,z,c) = v;
}

...
static inline int tiff_channel_msize(Tiff_Channel *channel)
{ if (channel->interpretation == CHAN_MAPPED)
    return (3*sizeof(unsigned short)*(1 << channel->scale));
  else
    return (0);
}
...
static inline void allocate_tiff_channel_map(Tiff_Channel *tiff_channel, int msize, char *routine)
{ _Tiff_Channel *object  = (_Tiff_Channel *) (((char *) tiff_channel) - Tiff_Channel_Offset);
  if (object->msize < msize)
    { if (object->msize == 0)
        object->tiff_channel.map = NULL;
      object->tiff_channel.map  = Guarded_Realloc(object->tiff_channel.map,msize,routine);
      object->msize = msize;
    }
}

static inline Tiff_Channel *new_tiff_channel(int psize, int msize, char *routine)
{ _Tiff_Channel *object;
  ...
  allocate_tiff_channel_map(&(object->tiff_channel),msize,routine);
...
{ Tiff_Channel *copy = new_tiff_channel(tiff_channel_psize(tiff_channel),tiff_channel_msize(tiff_channel),"Copy_Tiff_Channel");
  ...
  *copy = *tiff_channel;
  ...
  copy->map = temp.map;
  if (tiff_channel_msize(tiff_channel) != 0)
    memcpy(copy->map,tiff_channel->map,tiff_channel_msize(tiff_channel));
...
{ _Tiff_Channel *object  = (_Tiff_Channel *) (((char *) tiff_channel) - Tiff_Channel_Offset);
  ...
  if (object->msize > tiff_channel_msize(tiff_channel))
    { object->msize = tiff_channel_msize(tiff_channel);
      if (object->msize != 0)
        object->tiff_channel.map = Guarded_Realloc(object->tiff_channel.map,
                                                   object->msize,"Pack_Tiff_Channel");
      else
        { free(object->tiff_channel.map);
          object->msize = 0;
        }
    }
}
...
static inline Tiff_Image *new_tiff_image(int csize, char *routine)
{ _Tiff_Image *object;
...
{ Tiff_Image *copy = new_tiff_image(tiff_image_csize(tiff_image),"Copy_Tiff_Image");
...
int LZW_Encoder(unsigned char *stream, int stream_len, unsigned char *encode)
{ static int            firstime = 1;
...
{ static int             Max_Samples_Per_Pixel = 0;
  ...
  if (eflag != 0)
    return (error("IFD does not contain a valid tag for image width"));
  height = get_integer_tag(ifd,TIFF_IMAGE_LENGTH,&eflag);
  if (eflag != 0)
    return (error("IFD does not contain a valid tag for image height"));
  ...
  { int type, count;
    ...
    if (bits_per_sample != NULL)
      { if (type != TIFF_SHORT)
        ...
        if (samples_per_pixel != count)
          return (error("Length of bits_per_sample array not equal to samples_per_pixel tag"));
      }
    ...
    { int i, c;
      ...
      for (i = 0; i < samples_per_pixel; i++)
        { c = bits_per_sample[i];
          if (c > 32)
            return (error("# of bits in a sample is more than 32"));
  ...
  { int ctype, ccount;
    ...
    if (eflag != 0)
      return (error("Photometric interpretation tag is not present"));
    ...
    if (photometric == TIFF_VALUE_RGB)
      ...
      { if (color_map == NULL)
          return (error("Color map table is missing"));
        if (ctype != TIFF_SHORT)
          return (error("Color_map tag is not of type SHORT"));
        if (bits_per_sample[0] > 16)
          return (error("Color map over values with more than 16-bits"));
        if (ccount != 3*(1 << bits_per_sample[0]))
          return (error("Color map size doesn not match value range"));
        map_size = ccount*sizeof(unsigned short);
      }
    else if (photometric < 0 || photometric > TIFF_VALUE_TRANSPARENCY_MASK)
      return (error("Do not understand photometric_interpretation tag"));
  ...
  if (eflag != 0)
    ...
    return (error("Support only uncompressed and LZW compressed images"));
  ...
  { int type, count;
    ...
    if (countS == NULL)
      { countS  = (unsigned short *) Get_Tiff_Tag(ifd,TIFF_TILE_BYTE_COUNTS,&type,&count);
        if (countS == NULL)
          return (error("IFD does not contain a strip or tile image encoding"));
        theight = get_integer_tag(ifd,TIFF_TILE_LENGTH,&eflag);
        if (eflag != 0)
          return (error("Tile encoding does not contain a tile_height tag"));
        twidth  = get_integer_tag(ifd,TIFF_TILE_WIDTH,&eflag);
        if (eflag != 0)
          return (error("Tile encoding does not contain a tile_width tag"));
      }
    else
      { theight = get_integer_tag(ifd,TIFF_ROWS_PER_STRIP,&eflag);
        twidth  = width;
        if (eflag != 0)
          return (error("Strip encoding does not contain a strip_height tag"));
      }
  ...
  image = new_tiff_image(samples_per_pixel*sizeof(Tiff_Channel *),"Extract_Image_From_IFD");
  image->width           = width;
  image->height          = height;
  ...
  { int i;
    ...
    for (i = 0; i < samples_per_pixel; i++)
      { if (i == 0)
          channel[i] = new_tiff_channel(width*height*Bytes_Per_Sample[i],
                                        map_size,"Extract_Image_From_IFD");
        else
          channel[i] = new_tiff_channel(width*height*Bytes_Per_Sample[i],
                                        0,"Extract_Image_From_IFD");
  ...
  if (photometric == TIFF_VALUE_WHITE_IS_ZERO)
    ...
    { channel[0]->interpretation = CHAN_MAPPED;
      memcpy(channel[0]->map,color_map,map_size);
    }
...
{ Tiff_Image *image;
  ...
  image = new_tiff_image(10*sizeof(Tiff_Channel *),"Begin_Tiff_Image_Creation");
  image->width  = width;
  image->height = height;
...
{ Tiff_Channel *channel;
  ...
  image->channels[n] = channel = new_tiff_channel(0,0,"Add_Tiff_Image_Channel");
  ...
  allocate_tiff_channel_map(channel,tiff_channel_msize(channel),"Add_Tiff_Image_Channel");
...
{ int           b;								\
  ...
  for (i = 0; i < image->number_channels; i++)
    { int scale = image->channels[i]->scale;
      ...
      if (compression)
        byte_count = LZW_Encoder(source,byte_count,stream);
      else
        { if (source != stream)

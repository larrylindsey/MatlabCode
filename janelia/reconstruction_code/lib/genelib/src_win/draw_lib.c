...
//  fill_region is in water_shed.p becaue the waterhsed abstraction has
//    the working need to do the necessary floood fill algorithm.

extern fill_region(void (*painter)(int), Image *, int, int (*value)(int), int seed);
...
{ int  (*value)(int);
  ...
  fill_region(SETUP_PAINTER(canvas,brush),canvas,iscon4,value,seed);
}
...
{ Image *source = w->labels;
  ...
  fill_region(SETUP_PAINTER(canvas,brush),canvas,w->iscon4,is_eq,seed);
}

function gen_link_gph_overlap_hist_boundary_intr_hist_ncut_prealign_sift(config)
...
fprintf('START: gen_link_gph_overlap_hist_boundary_intr_hist_ncut_prealign_sift\n');
...
for i = 2:length(stack_config.case_ids)
  ...
  for tile_1 = 1:length(images_1)
    for tile_2 = 1:length(images_2)
      ...
      G(G==0) = -1;
      G(eye(size(G))>0) = 1;
      if(model_config.ncuts.is_only_positive)
        G(G<0) = 0;
      else
        G(G<0) = model_config.ncuts.neg_wt_factor*G(G<0);
      end
      [junk, junk, P] = ncuts_pos_neg_wts(G, model_config.ncuts.delta, ...
        model_config.ncuts.k, false);
      P = -P;
...
fprintf('done\n');
fprintf('STOP: gen_link_gph_overlap_hist_boundary_intr_hist_ncut_prealign_sift\n');

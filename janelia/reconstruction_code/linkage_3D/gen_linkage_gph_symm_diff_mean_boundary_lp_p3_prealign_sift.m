function gen_linkage_gph_symm_diff_mean_boundary_lp_p3_prealign_sift(config)
% gen_linkage_gph_symm_diff_mean_boundary_lp_p3_prealign_sift(config)
% generate linkage graph using optimization on symmetric difference and
% mean boundary values between linked segments.
...
fprintf('START: gen_linkage_gph_symm_diff_mean_boundary_lp_p3_prealign_sift\n');
...
for i = 2:length(stack_config.case_ids)
  ...
  for tile_1 = 1:length(images_1)
    for tile_2 = 1:length(images_2)
      ...
      mean_boundary_0 = collect_segment_pair_stats_mean_boundary(...
        uint32(seg_0), uint8(255-255*medfilt2(image_1_t, [3 3])));
      mean_boundary_0 = mean_boundary_0(min(mean_boundary_0(:,1:2), [], 2)>0, :);
      mean_boundary_1 = collect_segment_pair_stats_mean_boundary(...
        uint32(seg_1), uint8(255-255*medfilt2(image_2_t, [3 3])));
      mean_boundary_1 = mean_boundary_1(min(mean_boundary_1(:,1:2), [], 2)>0, :);
      ...
      F = zeros(n_node, n_node);
      fprintf('area of segments in seg_0\n');
      [seg_ids_0, area_0] = count_row_occurence(seg_0(:));
      area_0 = area_0(seg_ids_0>0);
      seg_ids_0 = seg_ids_0(seg_ids_0>0);
      F(sub2ind(size(F), seg_ids_0, seg_ids_0)) = area_0;
      
      fprintf('area of segments in seg_1\n');
      [seg_ids_1, area_1] = count_row_occurence(seg_1(:));
      area_1 = area_1(seg_ids_1>0);
      seg_ids_1 = seg_ids_1(seg_ids_1>0);
      F(sub2ind(size(F), seg_ids_1, seg_ids_1)) = area_1;
      
      fprintf('area of overlap\n');
      [seg_pair_ids, area_overlap] = count_row_occurence([seg_0(:), seg_1(:)]);
      area_overlap = area_overlap(min(seg_pair_ids,[], 2)>0);
      seg_pair_ids = seg_pair_ids(min(seg_pair_ids,[], 2)>0, :);
      F(sub2ind(size(F), seg_pair_ids(:,1), seg_pair_ids(:,2))) = -area_overlap;
      F(sub2ind(size(F), seg_pair_ids(:,2), seg_pair_ids(:,1))) = -area_overlap;
      ...
      B = zeros(size(F));
      B(sub2ind(size(B), mean_boundary_0(:,1), mean_boundary_0(:,2))) = ...
        mean_boundary_0(:,3)./mean_boundary_0(:,4);
      B(sub2ind(size(B), mean_boundary_1(:,1), mean_boundary_1(:,2))) = ...
        mean_boundary_1(:,3)./mean_boundary_1(:,4);
      B = max(B, B');
      f_threshold = model_config.mean_boundary_threshold;
      B(B<f_threshold) = 0;
      B(B>0) = B(B>0) - f_threshold;
      B = B/(255-f_threshold);
      
      A_2 = max(A_2, B~=0);
      
      G = -(F + B*model_config.delta + Lambda);
      G(G==0) = -1;

      fprintf('Dumping linear program.\n');
      lp_file_name = [config_global.temp_dir, 'joint_link_lp3.lp'];
      dump_linear_program_p3_v2(G, A_2, lp_file_name);
...
fprintf('done\n');
fprintf('STOP: gen_linkage_gph_symm_diff_mean_boundary_lp_p3_prealign_sift\n');

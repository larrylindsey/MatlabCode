...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[]){
    ...
    mexPrintf("\t5. bin size (uint8)\n");
    mexPrintf("\t6. piecewise mapping mask MxN uint16 matrix\n");
    mexPrintf("\t7. affine transformations 6xR double matrix\n");
		mexPrintf("output:\n");
  ...
  unsigned int bin_size = (unsigned int) *((unsigned char *) mxGetPr(prhs[4]));

  unsigned short int * map_mask = (unsigned short int *) mxGetPr(prhs[5]);
  double * transforms = mxGetPr(prhs[6]);
  ...
  Hash_UInt64_UInt32 label_pair_hist;
  {
    ...
    for(int i=0, x=0; x<width; x++){
      for(int y=0; y<height; y++, i++){
        ...
        l |= k;
        overlap_interior_hist[(boundary_map_0[i]+boundary_map_1[x1*height+y1])
                              /(2*bin_size)][l]++;
        label_pair_hist[l]++;
      }
    }

function gen_linkage_gph_symm_diff_lp_p3_prealign_sift(config)
% gen_linkage_gph_symm_diff_lp_p3_prealign_sift(config)
% generate linkage graph using optimization on symmetric difference between
% linked segments.
...
fprintf('START: gen_linkage_gph_symm_diff_lp_p3_prealign_sift\n');
...
%           image_2_t = images_2{tile_2};
      ...
      [seg_0, relabelling_0] = relabel_to_remove_nonexistent_labels(...
        segment_2D_label_map_1{tile_1}.label_map_t);
      seg_0_b_d = imdilate(double(seg_0==0), strel('disk', 8));
      seg_0(seg_0_b_d>0) = 0;
      [seg_1, relabelling_1] = relabel_to_remove_nonexistent_labels(...
        segment_2D_label_map_2{tile_2}.label_map_t);
      seg_1_b_d = imdilate(double(seg_1==0), strel('disk', 8));
      seg_1(seg_1_b_d>0) = 0;
...
%       F(abs(F)<model_config.min_abs_F_threshold)=0;
      
      lambda = model_config.lambda;
      fprintf('lambda: %d\n', lambda);
      Lambda = zeros(size(F))+0.1;
      ...
      dump_linear_program_p3_v2(F, A_2, lp_file_name);
...
fprintf('done\n');
fprintf('STOP: gen_linkage_gph_symm_diff_lp_p3_prealign_sift\n');

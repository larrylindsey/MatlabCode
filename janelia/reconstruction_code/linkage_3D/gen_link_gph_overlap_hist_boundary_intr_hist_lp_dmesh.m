    ...
    for tile_2 = 1:length(image_prefixes_2)
      ...
      if(isfield(linkage_config, 'modified_segment') || ...
        ...
        [features, label_pairs, label_pairs_original] = ...
          ...
          transforms_tp, transforms_tp_rev, linkage_model.feature_config, ...
      ...
      [area_overlap, label_pairs_overlap] = ...
        get_linkage_features_overlap_area_shrink_LS(...
        seg_1_original, ones(size(seg_1_original)), ...
        seg_2_original, ones(size(seg_2_original)), ...
        transforms_tp, transforms_tp_rev);
      ...
      [features, label_pairs] = get_linkage_features_overlap_area_shrink_LS(...
        ...
        transforms_tp, transforms_tp_rev);
      if(~isempty(features))
        link_confidence = (features(:,1)>10000).*(2 + features(:,1)/10000);
      ...
      if(~isempty(links_temp))
        ...
        interior_hist_t = collect_segment_stats_interior_hist(...
          uint32(seg_1_original), uint8(255*images_1{tile_1}), ...
          uint8(linkage_model.feature_config.interior_hist.bin_size));
        interior_hist = [];
...
%         end
      ...
      if(~isempty(links_boost_area))
        links_temp = [links_temp; links_boost_area(links_boost_area(:,3)>2.75,:)]; %#ok<AGROW>
      end
      ...
      if(~isempty(label_pairs_overlap))
        label_pairs_overlap = [label_backref_1(1+label_pairs_overlap(:,1))', ...
          label_backref_2(1+label_pairs_overlap(:,2))', area_overlap(:,1)]; %#ok<NASGU>
      end
      ...
      if(isfield(linkage_config, 'include_proofread') && ...
        ...
        links_3D_p = include_proofread_linkage_graph(...
          image_prefixes_1{tile_1}, image_prefixes_2{tile_2}, ...
          linkage_config, links_3D_p, config);
      end
      
      if(~isempty(links_3D_p))
        fprintf('saving linkage graph:\n%s\n', file_name_linkage_graph);
        save2(file_name_linkage_graph, 'links_3D_p', 'label_pairs_overlap');
      end
    end
  end

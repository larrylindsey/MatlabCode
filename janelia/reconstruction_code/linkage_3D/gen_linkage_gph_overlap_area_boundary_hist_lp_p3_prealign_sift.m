function gen_linkage_gph_overlap_area_boundary_hist_lp_p3_prealign_sift(config)
% gen_linkage_gph_overlap_area_boundary_hist_lp_p3_prealign_sift(config)
...
fprintf('START: gen_linkage_gph_overlap_area_boundary_hist_lp_p3_prealign_sift\n');
...
version_name = 'overlap_area_boundary_hist';
...
for i = 2:length(stack_config.case_ids)
  ...
  for tile_1 = 1:length(images_1)
    for tile_2 = 1:length(images_2)
      ...
      if(size(features_linkage, 2) ~= ...
          classifier_model.feature_config.overlap_area.n_dimension)
        error('Linkage feature dimensions during training and testing don''t match.');
      end
      confidence_linkage = ...
      ...
      if(size(features, 2) ~= ...
          classifier_model.feature_config.boundary_hist.n_dimension)
        error('Boundary feature dimensions during training and testing don''t match.');
      end
...
fprintf('done\n');
fprintf('STOP: gen_linkage_gph_overlap_area_boundary_hist_lp_p3_prealign_sift\n');

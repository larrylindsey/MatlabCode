...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[]){
  ...
  Hash_UInt32_UInt32 adjacency;
  for(int i=0; i<n_node; i++)
    for(int j=0; j<n_node; j++)
      if(i!=j)
        adjacency[get_adj_id_32(i,j)] = adjacency_0[i + j*n_node];
  /*
    for(int i=0; i<n_node; i++)
    for(int j=0; j<n_node; j++){
  ...
  */
  ...
  std::vector<Pair_UInt32> edges;
  {
    ...
    for(hp=triplet_id.begin(); hp!=triplet_id.end(); hp++){
      ...
      for(d=0; d<n_node; d++){
        ...
        if(adjacency[get_adj_id_32(d,a)]==1 &&
           adjacency[get_adj_id_32(d,b)]==1){
          if(n1>l){
            edges.push_back(Pair_UInt32(triplet_id[l], triplet_id[n1]));
          }
        }
        if(adjacency[get_adj_id_32(d,a)]==1 &&
           adjacency[get_adj_id_32(d,c)]==1){
          if(n2>l){
            edges.push_back(Pair_UInt32(triplet_id[l], triplet_id[n2]));
          }
        }
        if(adjacency[get_adj_id_32(d,c)]==1 &&
           adjacency[get_adj_id_32(d,b)]==1){
          if(n3>l){
  ...
  // Simple region growing
  {
    ...
    // add first adjacency you find
    {
      ...
      unsigned int a = get_adj_id_32(i, j);
      q.push(a);
    }
    
    while(!q.empty()){
      unsigned int a_ij = q.front();
      q.pop();
      if(adj_flag[a_ij]>0) // already processed - skip
        continue;
      unsigned int i = a_ij >> 16;
      unsigned int j = a_ij & 0x0000ffff;
      if(i==j)
        continue;
#ifdef __DEBUG__
      mexPrintf("i: %d, j: %d\n", i, j);
      ...
      // add all unprocessed neighbors of this vertex to Q
      for(unsigned int k=0; k<n_node; k++){
        if(k==i || k==j)
          continue;
        unsigned int a_ik = get_adj_id_32(i, k);
        if(adjacency[a_ik]>0 && adj_flag[a_ik]==0){
#ifdef __DEBUG__
          mexPrintf("adding adj(%d,%d) to q.\n", i, k);
#endif
          q.push(a_ik);
        }
        
        unsigned int a_jk = get_adj_id_32(j, k);
        if(adjacency[a_jk]>0 && adj_flag[a_jk]==0){
#ifdef __DEBUG__
          mexPrintf("adding adj(%d,%d) to q.\n", j, k);
#endif
          q.push(a_jk);
...
#endif
      if(q.empty()){ // add one un-processed vertex if exists
        Hash_UInt32_UInt32::iterator hp;
        for(hp=adjacency.begin(); hp!=adjacency.end(); hp++){
          if((*hp).second!=0 && adj_flag[(*hp).first]==0){
            q.push((*hp).first);
            break;
          }
        }
  ...
  plhs[0] = mxCreateDoubleMatrix(n_node, n_node, mxREAL);
  {
    ...
    for(hp=adjacency.begin(); hp!=adjacency.end(); hp++){
      if((*hp).second==1 && adj_flag[(*hp).first]==1){

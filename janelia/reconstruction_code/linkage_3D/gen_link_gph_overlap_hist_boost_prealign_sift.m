function gen_link_gph_overlap_hist_boost_prealign_sift(config)
...
fprintf('START: gen_link_gph_overlap_hist_boost_prealign_sift\n');
...
for tile = 1:length(images_1)
  ...
  if(isfield(feature_config, 'filter_version') && ...
    ...
    images_1{tile} = filter_image2(images_1{tile}, feature_config.filter_version);
  end
  ...
  if(isfield(feature_config, 'seg_filter_version') && ...
    ...
    segment_2D_label_map_1{tile}.label_map = filter_image2(...
      segment_2D_label_map_1{tile}.label_map, ...
      feature_config.seg_filter_version); %#ok<AGROW>
  end
  ...
  if(version_id==2)
    fprintf('shrinking segmentation using level sets\n');
    phi_shrink_LS = filter_image(images_1{tile}, ...
      feature_config.phi_shrink_LS_filter_version, config, image_prefixes_1{tile});
    segment_2D_label_map_1{tile}.label_map = shrink_segment_levelset_ChanVese(...
      segment_2D_label_map_1{tile}.label_map, phi_shrink_LS); %#ok<AGROW>
  end
end
...
for i = 2:length(stack_config.case_ids)
  ...
  for tile = 1:length(images_2)
    ...
    if(isfield(feature_config, 'filter_version') && ...
        ~isempty(feature_config.filter_version))
      images_2{tile} = filter_image2(images_2{tile}, feature_config.filter_version);
    end
    ...
    if(isfield(feature_config, 'seg_filter_version') && ...
        ~isempty(feature_config.seg_filter_version))
      segment_2D_label_map_2{tile}.label_map = filter_image2(...
    ...
    if(version_id==2)
      fprintf('shrinking segmentation using level sets\n');
      phi_shrink_LS = filter_image(images_2{tile}, ...
        feature_config.phi_shrink_LS_filter_version, config, image_prefixes_2{tile});
      segment_2D_label_map_2{tile}.label_map = shrink_segment_levelset_ChanVese(...
        segment_2D_label_map_2{tile}.label_map, phi_shrink_LS); %#ok<AGROW>
    end
  end
  ...
  for tile_1 = 1:length(images_1)
    for tile_2 = 1:length(images_2)
      ...
      weak_learner = tree_node_w(classifier_model.model_config.tree_depth); %#ok<NASGU>
      confidence_linkage = ...
        Classify(classifier_model.classifier.GLearners, ...
        classifier_model.classifier.GWeights, ...
        features_linkage');
      
      links_3D_p{tile_1, tile_2} = [overlap_hist_t(:, 1:2), confidence_linkage']; %#ok<AGROW>
    end
  end
...
fprintf('done\n');
fprintf('STOP: gen_link_gph_overlap_hist_boost_prealign_sift\n');

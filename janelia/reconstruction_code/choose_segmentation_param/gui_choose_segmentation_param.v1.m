...
for index = 1:length(stack_config.case_ids)
  ...
  for tile_id = 1:length(images)
    ...
    while(current_node_id>0)
      ...
      subplot('position', [MARGIN, MARGIN, display_width, 1]);
      imshow(image);
      subplot('position', [MARGIN + MAX_WIDTH, MARGIN, display_width, 1]);
      imshow(image);
      colormap('gray');
      hold on;
      for i = 1:2
        ...
        plot(px, py, ['.', choose_config.plot_colors{i}], 'MarkerSize', 2); 
      end
      hold off;
      user_choice = 0;
      while(user_choice==0)

function seg_fly_larva_1st_Instar_brain_test_newPrep_2(module_id)
% seg_fly_larva_1st_Instar_brain_test_newPrep(module_id)
% 2D segmentation of images from fly larva 1st Instart Brain Test new-prep
...
% v7  11032008  fly larva 1st Instart Brain Test OreR
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) stack name
config.stack.name = '1st_Instar_brain_test_newPrep';   
% (b) image name:
...
config.superpixel.f_thresholds = 0.4;
...
% detection).
config.superpixel.filter_version = 'v_heq_mf3';
...
config.superpixel_2_seg(1).superpixel_suffix = '.gs_l_T0.4_L400.v_heq_mf3';
% (c) thresholds on the boundary field
config.superpixel_2_seg(1).f_threshold_seq = 0.4:0.01:0.74;
% (c) thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(1).save_f_thresholds = 0.7:0.01:0.74;
...
% % use superpixel segmentation parameters from previous step
% For grayscale-ladder .gs_l_T<f_threshold>_L<area_threshold>.<filter_version>_<mitochondria m<threshold_d<amount of erosion>>
config.superpixel_2_seg(2).superpixel_suffix = '.gs_amb_sp_T0.72_0.71_b25.gs_l_T0.4_L400.v_heq_mf3.v_heq_mf3';
% '.gs_l_T0.47_L100.v_heq_mf3_e2_m0_d2_a500'; 
% (c) thresholds on the boundary field
config.superpixel_2_seg(2).f_threshold_seq = 0.68:0.01:0.72; 
% 0.48:0.01:0.9;
% (c) thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(2).save_f_thresholds = 0.68:0.01:0.72;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel_2_seg(2).filter_version = 'v_heq_mf3';
% () whether to display intermediate results
...
% For 1900x
config.superpixel_2_seg(3).superpixel_suffix = '.gs_amdb_sp_T0.72_0.71_b25.gs_amb_sp_T0.72_0.71_b25.gs_l_T0.4_L400.v_heq_mf3.v_heq_mf3.v_heq_mf3';
% (c) thresholds on the boundary field
config.superpixel_2_seg(3).f_threshold_seq = 0.01:0.05:0.61;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
% (c) thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(3).save_f_thresholds = config.superpixel_2_seg(3).f_threshold_seq(end);

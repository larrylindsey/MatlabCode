function config = mitochondria_2007KHLateral_s09_ihg(module_id)
% config = mitochondria_2007KHLateral_s09_ihg(module_id)
% Mitochondria detection on 2007KH Lateral S09 images.
%
% module_id
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) stack name
config.stack.name = 'mitochondria_2007KHLateral.s09';
% (b) image name:
...
% ".tif".
config.stack.image_prefix = 's09%03d';  
config.stack.image_suffix = '.tif';
% (c) slice ids: If the images are a001.tif, ..., a010.tif then the
% case_ids are 1,2,...,10.
config.stack.case_ids = 2:8;
% (d) ROI: Region Of Interest for the stack. If the entire slice image is
% to be processed then assign empty ([]). Otherwise get_roi(config);
% config.stack.roi = [];
% config.stack.roi = get_roi(config);
config.stack.roi.xmin = 465;
config.stack.roi.xmax = 1849;
config.stack.roi.ymin = 1197;
config.stack.roi.ymax = 2581;

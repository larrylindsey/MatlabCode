function seg_3rd_instar_leg_neuropil_rick_11162008_4800x(module_id)
% seg_3rd_instar_leg_neuropil_rick_11162008_4800x(module_id)
% 2D segmentation of images from 3rd_Instar_leg_neuropil.rick.11162008
% 4800x res.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) stack name
config.stack.name = '3rd_Instar_leg_neuropil.rick.11162008';   
% (b) image name:
...
config.reconstruction.name = '11182008';
...
config.superpixel.f_thresholds = [0.35, 0.4];
% (c) minimum area of a segment - below this they merged with neighbors
config.superpixel.area_thresholds = 400; % 400
...
config.superpixel_2_seg(1).superpixel_suffix = '.gs_l_T0.4_L400.v_heq';
% (c) thresholds on the boundary field
config.superpixel_2_seg(1).f_threshold_seq = 0.4:0.01:0.7;
% (c) thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(1).save_f_thresholds = 0.6:0.01:0.7;
...
% % use superpixel segmentation parameters from previous step
% For grayscale-ladder .gs_l_T<f_threshold>_L<area_threshold>.<filter_version>_<mitochondria m<threshold_d<amount of erosion>>
config.superpixel_2_seg(2).superpixel_suffix = '.gs_amb_sp_T0.66_0.65_b25.gs_l_T0.4_L400.v_heq.v_heq';
% '.gs_l_T0.47_L100.v_heq_mf3_e2_m0_d2_a500'; 
% (c) thresholds on the boundary field
config.superpixel_2_seg(2).f_threshold_seq = 0.6:0.01:0.7; 
% 0.48:0.01:0.9;
% (c) thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(2).save_f_thresholds = 0.6:0.01:0.7;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
% For 1900x
config.superpixel_2_seg(3).superpixel_suffix = '.gs_amdb_sp_T0.65_0.64_b0.gs_amb_sp_T0.66_0.65_b25.gs_l_T0.4_L400.v_heq.v_heq.v_heq';
% (c) thresholds on the boundary field
config.superpixel_2_seg(3).f_threshold_seq = 0.01:0.05:0.6;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
% (c) thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(3).save_f_thresholds = 0.41;

function gen_link_gph_overlap_hist_boundary_intr_hist_lp_dmesh(config)
...
if(linkage_config.is_verbose)
  fprintf('START: gen_link_gph_overlap_hist_boundary_intr_hist_lp_dmesh\n');
end
...
linkage_model = load('../../medulla.HPF.Leginon.3500x.zhiyuan.fall2008/linkage_3D/link_model.boost.40.2.overlap_hist_shrink_LS.10.db625959730279_40.mat');
  ...
  for tile_1 = 5%1:length(image_prefixes_1)
    for tile_2 = 9%1:length(image_prefixes_2)
      ...
      if(isfield(linkage_config, 'modified_segment') || ...
        ...
        [seg_1_original, relabelling_1] = relabel_to_remove_nonexistent_labels(...
          segment_2D_label_map_1{tile_1}.original_label_map);
        [seg_2_original, relabelling_2] = relabel_to_remove_nonexistent_labels(...
          segment_2D_label_map_2{tile_2}.original_label_map);
        relabelling_2(relabelling_2>0) = relabelling_2(relabelling_2>0) + ...
          max(relabelling_1);
        seg_2_original(seg_2_original>0) = ...
          seg_2_original(seg_2_original>0) + max(relabelling_1);
        seg_1 = relabelling_1(1+segment_2D_label_map_1{tile_1}.label_map);
        seg_2 = relabelling_2(1+segment_2D_label_map_2{tile_2}.label_map);
      else
        [seg_1_original, relabelling_1] = relabel_to_remove_nonexistent_labels(...
        ...
        seg_2_original(seg_2_original>0) = ...
          seg_2_original(seg_2_original>0) + max(relabelling_1);
        seg_1 = seg_1_original;
        seg_2 = seg_2_original;
      end
      relabelling_backref_1 = [];
      relabelling_backref_1(relabelling_1(relabelling_1>0)) = ...
        find(relabelling_1)-1; %#ok<AGROW>
      relabelling_backref_2 = [];
      relabelling_backref_2(relabelling_2(relabelling_2>0)) = ...
        find(relabelling_2)-1; %#ok<AGROW>
...
%           error('Link feature dimensions don''t match during training and testing.');
%         end
        link_confidence = ...
      ...
      if(~isempty(features))
        ...
        links_temp_area = [label_pairs, link_confidence];
        links_temp = [links_temp; links_temp_area]; %#ok<AGROW>
      end
      
%       if(false && ~isempty(links_temp))
...
%           features');
%         
%         boundary_hist_1 = boundary_hist_1(confidence_boundary_1<0, :);
%         confidence_boundary_1 = confidence_boundary_1(confidence_boundary_1<0);
%         
...
%           features');
%         
%         boundary_hist_2 = boundary_hist_2(confidence_boundary_2<0, :);
%         confidence_boundary_2 = confidence_boundary_2(confidence_boundary_2<0);
% 
%         links_temp = links_temp(links_temp(:,3)>=1, :);
% 
%         min_node_id_1 = 1;
%         max_node_id_1 = max(relabelling_1);
%         min_node_id_2 = max_node_id_1 + 1;
%         max_node_id_2 = max(relabelling_2);
...
%         end
%         
%         cc_id = get_connected_components_c(sparse(R), 0.9);
%         links_temp = [...
...
%       end
      ...
      if(~isempty(links_temp))
        ...
        links_temp = [relabelling_backref_1(links_temp(:,1))', ...
          relabelling_backref_2(links_temp(:,2))', links_temp(:,3)];
      end
      
      if(isfield(linkage_config, 'modified_segment') || ...
          isfield(apply_config, 'retain_links_subset'))
        links_temp = [links_temp; ...
          relabelling_backref_1(label_pairs_original(:,1))', ...
          relabelling_backref_2(label_pairs_original(:,2))', ...
          zeros(size(label_pairs_original,1),1)-2^14]; %#ok<AGROW>
      end
...
if(linkage_config.is_verbose)
  fprintf('STOP: gen_link_gph_overlap_hist_boundary_intr_hist_lp_dmesh\n');
end

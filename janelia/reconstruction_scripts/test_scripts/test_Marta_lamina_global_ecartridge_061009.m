function test_Marta_lamina_global_ecartridge_061009(module_id, case_ids)
% Marta_lamina_F5_05012009(module_id)
...
% config.stack.image_structure = 'global_e_hcartridge_lacie.xml';
% config.stack.image_structure_differential = 'global_ecartridge_lacie.xml';

 config.stack.image_structure = 'global_ecartridge_lacie.xml';
 
% % * slice ids: specify the z-plane numbers.
if(nargin>=2)
  config.stack.case_ids = case_ids;
else
  config.stack.case_ids = 0:627;
end
...
% Print messages on prompt?
config.align.in_section_tile_pair_align.deformable_mesh.is_verbose = false;
% Display results using figures?
config.align.in_section_tile_pair_align.deformable_mesh.is_verbose_figures = false;
...
config.align.linkage_align.deformable_mesh.is_verbose_figures=false;
% Filter images before feeding to SIFT feature point detector. By default
% no filtering or when defined empty.
...
%%%%%%
% 2.3 For aligning tiles in adjacent sections (joint alignment) for linkage
...
% Display results using figures?
config.align.in_section_align.deformable_mesh.is_verbose_figures = true;
% Calculate adjacency matrix from trakEM2 tiling
...
% exceptions{2,1} = 201:210; exceptions{2,2} = '-plan_B_method=[]';
...
config.align.global_align.display.save_as_stack_file_name = ...
  [ '/home/marta/temp/lamina_align_test_global_', num2str(config.stack.case_ids(1)), ...
...
exceptions{2,1} = [77,76]; exceptions{2,2} = '-SCALE=1.10';

%exceptions{3,1} = [283,284]; exceptions{3,2} = '-NCP=200000 -SCALE=0.92';
%exceptions{4,1} = [284,283]; exceptions{4,2} = '-NCP=200000 -SCALE=1.08';

  % exceptions{3,1} = [362,363]; exceptions{3,2} = '-FFM=0.5 -DFT=0.21 -NCP=200000 -FYL=0.48';
  % exceptions{4,1} = [363,362]; exceptions{4,2} = '-FFM=0.5 -DFT=0.21 -NCP=200000 -FYL=0.48';
% 
   exceptions{3,1} = [362,363]; exceptions{3,2} = '-DIT=0.2 -DFT=0.25 -FYL=0.45 -FFM=0.600 -MMA=100000 -NCP=200000';
   exceptions{4,1} = [363,362]; exceptions{4,2} = '-DIT=0.2 -DFT=0.25 -FYL=0.45 -FFM=0.600 -MMA=100000 -NCP=200000';

 

config.align.dmesh.exceptions=exceptions;
...
config.reconstruction.name = '07292009';
...
config.superpixel(1).f_thresholds = 0.25;
% (c) minimum area of a segment - below this they merged with neighbors
config.superpixel(1).area_thresholds = 150;
...
config.superpixel(1).mitochondria_confidence_threshold = 0.0;
% (f) The thresholded mitochondria mask is eroded to ensure that correct
% boundaries are not obliterated
config.superpixel(1).mitochondria_erosion = 5;
...
% detection).
config.superpixel(1).filter_version = 'v_heq2_mf2';
% () whether to display intermediate results
config.superpixel(1).is_verbose = false;
...
% to load from. The proofread superpixel maps are read with suffix
 %[ '.prfrdasp.', config.superpixel(1).include_proofread.proofread_name];
...
config.superpixel(1).include_proofread.proofread_name = 'test';

% (a) Algorithm to be used - must match with the function called
config.superpixel(2).method = 'grayscale_AglMeanB';
% () Superpixel method
config.superpixel(2).superpixel_method='grayscale_ladder';
%config.superpixel(2).superpixel_method = ;
...
%config.superpixel(2).superpixel_suffix = '.gs_l_T0.25_L100.v_mf2'; 
config.superpixel(2).superpixel_suffix=['.gs_l_T0.25_L150', ...
      '.v_heq2_mf2_f', ...
     '.v_heq2_mf2'];
% (c) thresholds on the boundary field
config.superpixel(2).f_threshold_seq = 0.25:0.01:0.66;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(2).save_f_thresholds = [0.64, 0.66];
...
config.superpixel(2).mitochondria_confidence_threshold = 0.0;
% () The thresholded mitochondria mask is eroded to ensure that correct
% boundaries are not obliterated
config.superpixel(2).mitochondria_erosion = 10;
...
% detection).
config.superpixel(2).filter_version = 'v_heq2_mf2';
% () whether to display intermediate results
config.superpixel(2).is_verbose = true;
config.superpixel(2).is_verbose_figures = false;

%%%%%%%%%%%%%%%%%%%%%%%%%%
% 8.2 Superpixel to segment - agglomerative median boundary
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) Algorithm to be used - must match with the function called
config.superpixel(3).method = 'grayscale_AglMedianB';
% () Superpixel method
config.superpixel(3).superpixel_method = 'grayscale_AglMeanB';
% (b) Superpixel version to be used as basis
% % use superpixel segmentation parameters from previous step
% For grayscale-ladder .gs_l_T<f_threshold>_L<area_threshold>.<filter_version>_<mitochondria m<threshold_d<amount of erosion>>
...
% (c) thresholds on the boundary field
config.superpixel(3).f_threshold_seq = 0.50:0.01:0.66; 
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(3).save_f_thresholds = [0.64, 0.66];
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
config.superpixel(3).length_threshold_seq = ...
  25*(config.superpixel(3).f_threshold_seq>0.65);
% () Whether mitochondria are to be used to suppress false boundaries
...
% () The thresholded mitochondria mask is eroded to ensure that correct
% boundaries are not obliterated
config.superpixel(3).mitochondria_erosion = 10;
% () Whether to use vesicle detection to suppress false boundaries
...
% () Image filtering before performing segmentation (affects boundary
% detection)
config.superpixel(3).filter_version = 'v_heq2_mf2';
% () whether to display intermediate results
config.superpixel(3).is_verbose = true;
config.superpixel(3).is_verbose_figures = false;
...
% proofread name (same as used for config.proofreader.import.proofread_name
config.superpixel_2_seg(1).include_proofread.proofread_name = 'test';
...
config.linkage.train.manual_annotation_file = ...
  '/groups/visitors/home/albam/em_reconstruction/manual_annotations/marta.06032008.wcat.no_glia.mat'; 
...
config.linkage.feature.type = 'intensity_pair_hist_v2c';%'anon';%
...
config.linkage.model.type = 'boost';%'proofread';%
...
config.linkage.apply.model_suffix = '.01192009';%'.v0';%
% (j) Verbose
...
% proofread name (same as used for config.proofreader.import.proofread_name
config.linkage.include_proofread.proofread_name = 'proofinblocks';
...
%       config.proofreader.roi.xmin = 2456;
%       config.proofreader.roi.xmax =4504;
%       config.proofreader.roi.ymin = 2392;
%       config.proofreader.roi.ymax = 4936;

%      config.proofreader.roi.xmin = 2004;
%      config.proofreader.roi.xmax =3918;
%      config.proofreader.roi.ymin = 1986;
%      config.proofreader.roi.ymax = 3864;

%     config.proofreader.roi.xmin = 1188;
%     config.proofreader.roi.xmax =3936;
%     config.proofreader.roi.ymin = 2016;
%     config.proofreader.roi.ymax = 4698;


%   config.proofreader.roi.xmin = 1464;
%  config.proofreader.roi.xmax =4784;
%  config.proofreader.roi.ymin = 1704;
%  config.proofreader.roi.ymax = 4608;

% config.proofreader.roi.xmin = 1616;
% config.proofreader.roi.xmax = 4600;
% config.proofreader.roi.ymin = 1656;
% config.proofreader.roi.ymax = 4216;

% config.proofreader.roi.xmin = 1537;
% config.proofreader.roi.xmax = 4266;
% config.proofreader.roi.ymin = 1837;
% config.proofreader.roi.ymax = 4386;

function reconstruction_fly_larva_FIB_5x5x5nm_ssTEM_pipeline(module_id, case_ids)
...
% config.linkage.train.save_suffix = '.08102009';
...
% config.linkage.apply.model_suffix = '.08102009';
% Type of classifier
config.linkage.model.type = 'lp_p3';
config.linkage.model.lambda = 1250; % 1250
config.linkage.model.min_abs_F_threshold = 0;
% Version name for the linkage model
...
% The model version to be used when applying the linkage on the stack
config.linkage.apply.model_suffix = config.linkage.train.save_suffix;
% Whether to shrink wrap the volume - make all bodies to be connected to
% the perimeter
config.linkage.apply.shrink_wrap.is_enabled = true;
% Linkage threshold upto which links may be forced. Below this, a body is
% not linked with another body even if it is not connected to the perimeter
% shell.
config.linkage.apply.shrink_wrap.linkage_threshold = 0.25;

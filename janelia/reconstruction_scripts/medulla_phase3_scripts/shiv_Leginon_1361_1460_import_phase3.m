function config = shiv_Leginon_1361_1460_import_phase3(config, case_ids, is_verbose, is_verbose_figures)
...
config.coarse_al.scale = 10; % scale down factor for al_coarse.tif file.
...
config.reconstruction.name = 'ms3_1361.1460_4k.4k'; %'ms3_611.710_4k.4k';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
if(length(config.superpixel)<2)
  config.superpixel = repmat(config.superpixel(1), [2 1]);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
if(length(config.superpixel_2_seg)<2)
  config.superpixel_2_seg = repmat(config.superpixel_2_seg(1), [2 1]);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
config.superpixel_2_seg(1).f_threshold_seq = 0.01:0.01:0.02;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(1).save_f_thresholds = 0.02;
% Whether to save the segmentation overlaps as TIF files for viewing.
...
% (c) thresholds on the boundary field
config.superpixel_2_seg(2).f_threshold_seq = 0.01:0.01:0.02; %0.01:0.01:0.08;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(2).save_f_thresholds = 0.02;
% Whether to save the segmentation overlaps as TIF files for viewing.
...
config.linkage.apply.model_suffix = '.09042009.40.2';
% (j) Verbose
config.linkage.apply.is_verbose = false;
% Whether to simply delete all linkage graphs in the specified sections and
% _not_ create new ones.
config.linkage.apply.delete_linkage_graph_dont_create = false; % put true when you want to delete the files.
...
config.proofreader.roi.xmin = 6681;
config.proofreader.roi.ymin = 7401;
config.proofreader.roi.xmax = 10680;
config.proofreader.roi.ymax = 11400;
config.proofreader.export.is_verbose_figures = false;

% Suffix for saving imported data
config.proofreader.import.proofread_name = 'concatenate_161_1460';
% Print messages
config.proofreader.import.is_verbose = is_verbose;
% Display intermediate result figures
config.proofreader.import.is_verbose_figures = is_verbose_figures;
% Data-to-be-proofread directory from which Raveler loaded data. This
% directory will have information about the transformations used for the
% reconstruction.
config.proofreader.import.Raveler.to_be_proofread_dir = ...
  '/groups/chklovskii/home/vitaladevunis/research/em_reconstruction_pipeline/reconstructions/medulla.HPF.Leginon.3500x.zhiyuan.fall2008/region.crop4_global_alignment_0161_1460.1361.1460.847838689109_40/';
%   '/groups/chklovskii/chklovskiilab/em_reconstruction/data_to_be_proofread/medulla.HPF.Leginon.3500x.zhiyuan.fall2008/region.crop4_global_alignment_0161_1460.1361.1460.847838689109_40/ms3_1361.1460_4k.4k/';
% Proofread-data directory to which Raveler exported the proofreading.
config.proofreader.import.Raveler.proofread_data_dir = ...
  '/groups/chklovskii/chklovskiilab/em_reconstruction/proofread_data/medulla.HPF.Leginon.3500x.zhiyuan.fall2008/region.crop4_global_alignment_0161_1460.1361.1460.847838689109_40/ms3_1361.1460_4k.4k_04052010/';
% File name format used by Raveler during export.

function segmentation_leginon_test_fly_larva_camera_swap_3500x_8amC(module_id)
% segmentation_leginon_test_fly_larva_camera_swap_3500x_8amC(module_id)
% 2D segmentation of stack
% leginon_tests_fly_larva_camera_swap_032409_3500x_0.8nmC to evaluate EM
% camera swap 3500x with new camera with Leginon. 
% Algo: Agglo. mean boundary for superpixels, followed by grayscale ladder 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) stack name
config.stack.name = 'leginon_tests_fly_larva_camera_swap_032409_3500x_0.8nmC';
% (b) image name:
...
% ".tif".
config.stack.image_prefix = '09mar23a_27986T2.4x4_00006gr_%05dex';  
config.stack.image_suffix = '.tif';
% (c) slice ids: If the images are a001.tif, ..., a010.tif then the
% case_ids are 1,2,...,10.
config.stack.case_ids = 32;
...
for f = 0.67
  s{end+1} = ['.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
    '_b', num2str(25*(f>0.65),'%d'), '.mf3.mf3'];
end
config.superpixel(2).superpixel_suffix = s; 
% () thresholds on the boundary field
config.superpixel(2).f_thresholds = 0.2:0.02:0.3;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
...
config.superpixel(2).area_thresholds = 400;
% () Whether mitochondria are to be used to suppress false boundaries
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(2).filter_version = 'mf3';
% () whether to display intermediate results
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% 6 Superpixel to segment - agglomerative boundary vs. interior values
...
config.superpixel_2_seg.superpixel_suffix = s; 
% (c) thresholds on the boundary field
config.superpixel_2_seg.f_threshold_seq = 0.1:0.01:0.18; % :0.02:0.7; % 0.002:0.07;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg.save_f_thresholds = 0.12:0.02:0.18;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
config.superpixel_2_seg.length_threshold_seq = ...
  2*(config.superpixel_2_seg.f_threshold_seq>0);
% () Maximum area of a segment, above this segments are not considered for
% merging.
config.superpixel_2_seg.max_area_threshold = 40000;
% () Whether mitochondria are to be excluded from image statistics
config.superpixel_2_seg.exclude_mitochondria_stat = false;
% () Whether mitochondria are to be used to suppress false boundaries
...
% () whether to display figures
config.superpixel_2_seg.is_verbose_figures = true;

function segmentation_leginon_test_fly_larva_Spirit_2900x_QtrSat(module_id)
% segmentation_leginon_test_fly_larva_Spirit_2900x_QtrSat(module_id)
% 2D segmentation of stack leginon_tests_fly_larva_Spirit_2900x_QtrSat to
% evaluate EM camera under Leginon.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) stack name
config.stack.name = 'leginon_tests_fly_larva_Spirit_2900x_QtrSat';
% (b) image name:
...
% ".tif".
% config.stack.image_prefix = '09jan14a_27985O2.QtrSat_00009gr_%05dex';  
config.stack.image_prefix = '09jan14a_27985O2.QtrSat_00004gr_%05dex';  
config.stack.image_suffix = '.tif';
% (c) slice ids: If the images are a001.tif, ..., a010.tif then the
% case_ids are 1,2,...,10.
config.stack.case_ids = 213; % 325;
...
config.superpixel_2_seg(1).superpixel_suffix = ...
  '.gs_l_T0.46_L150.v_heq_mf3';
% (c) thresholds on the boundary field
config.superpixel_2_seg(1).f_threshold_seq = 0.46:0.01:0.9;
...
config.superpixel_2_seg(2).superpixel_suffix = ...
  '.gs_amb_sp_T0.74_0.73_b25.gs_l_T0.46_L150.v_heq_mf3.v_heq_mf3';
% (c) thresholds on the boundary field
config.superpixel_2_seg(2).f_threshold_seq = 0.01:0.01:0.3;
% (c) thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.

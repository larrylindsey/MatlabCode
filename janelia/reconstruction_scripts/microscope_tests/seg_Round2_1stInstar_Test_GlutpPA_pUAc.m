function seg_Round2_1stInstar_Test_GlutpPA_pUAc(module_id)
% seg_Round2_1stInstar_Test_GlutpPA_pUAc(module_id)
% 2D segmentation of stack Round2_1stInstar_Test_Glut+PA_+UAc for
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) stack name
config.stack.name = 'Round2_1stInstar_Test_Glut+PA_+UAc';   
% (b) image name:
...
% ".tif".
config.stack.image_prefix = '27893R1.Section1.5000X.%d';
...
config.superpixel.f_thresholds = 0.3:0.1:0.7;
...
% detection).
config.superpixel.filter_version = 'v_heq'; % 'v_heq';
...
config.superpixel_2_seg(1).superpixel_suffix = '.gs_l_T0.5_L200.v_heq.v_heq'; % 2900x
% (c) thresholds on the boundary field
config.superpixel_2_seg(1).f_threshold_seq = 0.5:0.01:0.9;
% (c) thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(1).save_f_thresholds = 0.7:0.02:0.86;
...
% detection).
config.superpixel_2_seg(1).filter_version = 'heq_mf3';
% () whether to print messages
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel_2_seg(2).filter_version = 'mf3';
% () whether to display intermediate results

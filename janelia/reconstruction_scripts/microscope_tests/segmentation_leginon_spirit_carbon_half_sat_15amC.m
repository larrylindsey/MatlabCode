function segmentation_leginon_spirit_carbon_half_sat_15amC(module_id)
% segmentation_leginon_spirit_carbon_half_sat_15amC(module_id)
% 2D segmentation of stack leginon_spirit_carbon.half_sat.1.5nmC to
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) stack name
config.stack.name = 'leginon_spirit_carbon.half_sat.1.5nmC';
% (b) image name:
...
% ".tif".
config.stack.image_prefix = '09jan14a_27986U1.1.5nm_00013gr_%05dex';  
config.stack.image_suffix = '.tif';
% (c) slice ids: If the images are a001.tif, ..., a010.tif then the
% case_ids are 1,2,...,10.
config.stack.case_ids = 75;
...
% Whether to save the results as segmentations overlayed on images (TIF)
...
config.superpixel_2_seg(1).superpixel_suffix = ...
  '.gs_l_T0.42_L150.v_heq_mf3.v_heq_mf3';
...
config.superpixel_2_seg(2).superpixel_suffix = ...
  '.gs_amb_sp_T0.74_0.73_b25.gs_l_T0.42_L150.v_heq_mf3.v_heq_mf3.v_heq_mf3';
% (c) thresholds on the boundary field

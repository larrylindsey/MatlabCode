function test_sp_abif_seg_medulla_HPF_leginon(module_id)
...
% config.stack.case_ids = [4,5,6,7,9,11,14,16,17];
% Option 2: If the images correspond to a trakEM project then
% * specify the trakEM file structure xml file name
config.stack.image_structure = 'crop2.ms.8bit.3500x9x9_0681_0690.xml'; %'crop.ms.8bit.3500x9x9_0461_0470.xml'; %'crop.ms.8bit.3500x9x9_0681_0690.xml';
% * slice ids: specify the z-plane numbers.
config.stack.case_ids = 684:688; %[683, 688];
...
config.superpixel(1).f_threshold_seq = 0.1:0.01:0.58; % 0.1:0.01:0.9;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(1).save_f_thresholds = 0.45:0.01:0.58; %0.5:0.02:0.6;
...
config.superpixel(3).f_threshold_seq = 0.1:0.01:0.2; % :0.02:0.7; % 0.002:0.07;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(3).save_f_thresholds = 0.1:0.02:0.2;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
config.superpixel_2_seg(1).superpixel_suffix = '.gs_abif_sp_T0.24_0.23_b2.gs_l_sp_T0.2_L60.gs_amb_T0.6_0.59_b0.v_heq_o1_m0_d5_a40.v_heq2.v_heq2.v0'; %'.gs_l_sp_T0.2_L60.gs_amb_T0.6_0.59_b0.v_heq_o1_m0_d5_a40.v_heq2.v_heq2_mf2'; 
% (c) thresholds on the boundary field
config.superpixel_2_seg(1).f_threshold_seq = 0.5:0.01:0.7; %0.4:0.01:0.7;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(1).save_f_thresholds = 0.58:0.02:0.66;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
% % use superpixel segmentation parameters from previous step
% For grayscale-ladder .gs_l_T<f_threshold>_L<area_threshold>.<filter_version>_<mitochondria m<threshold_d<amount of erosion>>
config.superpixel_2_seg(2).superpixel_suffix = '.gs_amb_sp_T0.62_0.61_b0.gs_abif_sp_T0.24_0.23_b2.gs_l_sp_T0.2_L60.gs_amb_T0.6_0.59_b0.v_heq_o1_m0_d5_a40.v_heq2.v_heq2.v0.v_heq2';
% '.gs_l_T0.47_L100.v_heq_mf3_e2_m0_d2_a500'; 
% (c) thresholds on the boundary field
config.superpixel_2_seg(2).f_threshold_seq = 0.5:0.01:0.76;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(2).save_f_thresholds = 0.6:0.02:0.76;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
config.superpixel_2_seg(3).superpixel_suffix = ...
  '.gs_amdb_sp_T0.62_0.61_b0.gs_amb_sp_T0.62_0.61_b0.gs_abif_sp_T0.24_0.23_b2.gs_l_sp_T0.2_L60.gs_amb_T0.6_0.59_b0.v_heq_o1_m0_d5_a40.v_heq2.v_heq2.v0.v_heq2.v_heq2';
% (c) thresholds on the boundary field
config.superpixel_2_seg(3).f_threshold_seq = 0.1:0.01:0.7; % :0.02:0.7; % 0.002:0.07;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(3).save_f_thresholds = 0.22:0.04:0.42;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise

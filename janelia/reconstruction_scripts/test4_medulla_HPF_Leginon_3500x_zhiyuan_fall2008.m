function test4_medulla_HPF_Leginon_3500x_zhiyuan_fall2008(module_id)
...
config.stack.image_prefix = 'a.%03d'; %config.stack.image_structure = 'crop.ms.8bit.3500x9x9_0681_0690.xml';
config.stack.image_suffix = '.tif';
% * slice ids: specify the z-plane numbers.
config.stack.case_ids = 1:2; % 681:690;
...
% ".tif".
config.mitochondria.train.image_prefix = 'mitochondria.bin.%03d';
...
config.superpixel(1).f_threshold_seq = 0.1:0.01:0.7;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(1).save_f_thresholds = 0.5:0.02:0.6; %0.5:0.02:0.7;
...
% detection).
config.superpixel(1).filter_version = 'v_heq';
...
config.superpixel(2).superpixel_suffix = '.gs_amb_T0.52_0.51_b0.v_heq_o1_m0_d5_a40.v_heq'; 
% () thresholds on the boundary field
config.superpixel(2).f_thresholds = 0.2; %0.1;
% () minimum area of a segment - below this they merged with neighbors
config.superpixel(2).area_thresholds = 60; %100;
% () Whether mitochondria are to be used to suppress false boundaries
...
% () The thresholded mitochondria mask is eroded to ensure that correct
% boundaries are not obliterated
config.superpixel(2).mitochondria_erosion = 2;
% * Minimum of a thresholded connected component for it to be considered
% mitochondria
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(2).filter_version = 'v_heq';
% () whether to display intermediate results
...
config.superpixel_2_seg(1).superpixel_suffix = '.gs_l_sp_T0.2_L60.gs_amb_T0.52_0.51_b0.v_heq_o1_m0_d5_a40.v_heq.v_heq'; 
% (c) thresholds on the boundary field
config.superpixel_2_seg(1).f_threshold_seq = 0.5:0.01:0.76;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(1).save_f_thresholds = 0.6:0.02:0.7;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
% % use superpixel segmentation parameters from previous step
% For grayscale-ladder .gs_l_T<f_threshold>_L<area_threshold>.<filter_version>_<mitochondria m<threshold_d<amount of erosion>>
config.superpixel_2_seg(2).superpixel_suffix = '.gs_amb_sp_T0.64_0.63_b0.gs_l_sp_T0.2_L60.gs_amb_T0.52_0.51_b0.v_heq_o1_m0_d5_a40.v_heq.v_heq.v_heq';
% '.gs_l_T0.47_L100.v_heq_mf3_e2_m0_d2_a500'; 
% (c) thresholds on the boundary field
config.superpixel_2_seg(2).f_threshold_seq = 0.6:0.01:0.9;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(2).save_f_thresholds = 0.66:0.02:0.8;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
config.superpixel_2_seg(3).superpixel_suffix = ...
  '.gs_amdb_sp_T0.66_0.65_b25.gs_amb_sp_T0.64_0.63_b0.gs_l_sp_T0.2_L60.gs_amb_T0.52_0.51_b0.v_heq_o1_m0_d5_a40.v_heq.v_heq.v_heq.v_heq';
% (c) thresholds on the boundary field
config.superpixel_2_seg(3).f_threshold_seq = 0.1:0.01:0.8; % :0.02:0.7; % 0.002:0.07;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(3).save_f_thresholds = 0.16:0.02:0.3;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel_2_seg(3).filter_version = 'v0'; %'v_mf2';
% () whether to display intermediate results
...
config.linkage.train.save_suffix = '.10212008';
...
config.linkage.apply.model_suffix = '.10212008';
...
['.gs_abif_sp_T0.2_0.19_b10', config.superpixel_2_seg(end).superpixel_suffix, '.v_mf2'];
% (j) Verbose

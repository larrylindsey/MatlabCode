function reconstruction_ex_medulla_Leginon_681_684_subset(module_id)
% reconstruction_ex_medulla_Leginon_681_684_subset(module_id)
% Example of reconstruction script for pipeline as of Dec. 15 2008.
% Run the multi-tile reconstruction pipeline on a small subset medulla
% stack chklovskii/medulla/medulla.HPF.Leginon.3500x.zhiyuan.fall2008, as
% defined in test_0681_0684.xml.
%
% module_id
...
config.to_be_proofread.root_dir = ...
  '/groups/chklovskii/home/vitaladevunis/research/em_reconstruction_pipeline/data_to_be_proofread/';
...
% Option 2: Specify the region within the stack that is to be reconstructed
% If the images correspond to a trakEM project then
...
% this defaults to the config.stack.case_ids
config.region.case_ids = 682:683;
% * slice ids: specify the z-plane numbers that should be processed. These
% must be a subset of the region's case_ids.
config.stack.case_ids = 682:683;
...
config.superpixel(2).superpixel_suffix = '.gs_amb_T0.63_0.62_b0.v_heq_o1_m0_d5_a40_f.v_heq2'; %'.gs_amb_T0.6_0.59_b0.v_heq_o1_m0_d5_a40.v_heq'; 
% () thresholds on the boundary field
...
config.superpixel(3).superpixel_suffix = ...
  '.gs_l_sp_T0.2_L60.gs_amb_T0.63_0.62_b0.v_heq_o1_m0_d5_a40_f.v_heq2.v_heq2_f';
...
config.superpixel_choose.choice.seg_suffix = ...
  ['.gs_abif_sp_T0.18_0.17_b2', ...
  '.gs_l_sp_T0.2_L60', ...
  '.gs_amb_T0.63_0.62_b0.v_heq_o1_m0_d5_a40_f', ...
  '.v_heq2', ...
  '.v_heq2_f', ...
  '.v0_f'];
...
% (c) thresholds on the boundary field
config.superpixel_2_seg(1).f_threshold_seq = 0.55:0.01:0.7;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(1).save_f_thresholds = [0.60, 0.64, 0.68];
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(2).save_f_thresholds = [0.52, 0.58, 0.64];
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
config.proofreader.roi.xmin = 200;
config.proofreader.roi.ymin = 200;
config.proofreader.roi.xmax = 500;
config.proofreader.roi.ymax = 500;

function shiv_shinya_Leginon_test(module_id, case_ids, job_name)
% shinya_Leginon_test(module_id)
% Testing and debugging in medulla Leginon stack
%
% module_id
...
% Display results using figures?
config.align.global_align.deformable_mesh.is_verbose_figures = true;
% Calculate adjacency matrix from trakEM2 tiling
...
config.align.global_align.display.is_verbose = true;
% Whether to show the images inverted for easier assessment of alignment
...
config.reconstruction.name = 'test_pipeline_161_200';
...
config.superpixel(2).superpixel_suffix = '.gs_amb_T0.45_0.44_b0.v_heq_o1_m0_d5_a40_f.v_heq2'; %'.gs_amb_T0.6_0.59_b0.v_heq_o1_m0_d5_a40.v_heq'; 
% () thresholds on the boundary field
...
config.superpixel(3).superpixel_suffix = ...
  '.gs_l_sp_T0.2_L60.gs_amb_T0.45_0.44_b0.v_heq_o1_m0_d5_a40_f.v_heq2.v_heq2_f';
...
% (c) thresholds on the boundary field
config.superpixel_2_seg(1).f_threshold_seq = 0.45:0.01:0.7;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
...
config.linkage.train.save_suffix = '.02022009';
...
config.linkage.apply.model_suffix = '.02022009';
% (j) Verbose
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if(nargin<3)
  time_string = strrep(datestr(now), ' ', '_');
  job_name = [config.stack.name, '.', time_string];
end
config.job.name = job_name;
config_global.job.name = job_name;

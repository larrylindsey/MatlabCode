function Marta_lamina_J1_J2_03312009(module_id, case_ids)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) stack name
config.stack.name = 'J';%'lamina.OTO.shinya.leginon.3500x.July2008';
% (b) image name:
...
config.stack.image_structure = 'Jcartridge_elin.xml';
% * slice ids: specify the z-plane numbers.
if(nargin==1)
  config.stack.case_ids = 9:10;
else
...
config.align.global_align.display.save_as_stack_file_name = ...
  ['~/temp/lamina_align_test_J4_', num2str(config.stack.case_ids(1)), ...
...
config.reconstruction.name = '02092009';

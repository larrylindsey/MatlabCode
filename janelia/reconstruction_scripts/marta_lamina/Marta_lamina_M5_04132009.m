function Marta_lamina_M5_04132009(module_id, case_ids)
...
config.stack.image_structure = 'Mcartridge_elin_network.xml';
% * slice ids: specify the z-plane numbers.
if(nargin>=2)
  config.stack.case_ids = case_ids;
else
  config.stack.case_ids = 36:49;
end
...
config.align.global_align.display.save_as_stack_file_name = ...
  [ '~/temp/lamina_align_test_M5_', num2str(config.stack.case_ids(1)), ...
...
config.reconstruction.name = '04102009';

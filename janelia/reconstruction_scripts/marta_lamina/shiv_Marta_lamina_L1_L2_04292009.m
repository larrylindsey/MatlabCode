function shiv_Marta_lamina_L1_L2_04292009(module_id, case_ids, job_name)
...
%config_global.job.is_stand_alone = true;
...
config.stack.image_structure = 'Lcartridge_elin_network.xml';
...
% Whether to print intermediate messages
config.align.linkage_align.deformable_mesh.is_verbose = false;
% Filter images before feeding to SIFT feature point detector. By default
% no filtering or when defined empty.
...
config.align.global_align.deformable_mesh.optimization.levenberg_marquadt_rigid.lambda = 0.0001;
% Relative weight given to affine transformation criteria - same order of
% magnitude as the image dimensions.
config.align.global_align.deformable_mesh.optimization.levenberg_marquadt_rigid.alpha = 1000;
...
config.align.global_align.deformable_mesh.min_n_matches = 4;
...
config.align.global_align.display.save_as_stack_file_name = ...
   [ '~/temp/lamina_align_test_L1_', num2str(config.stack.case_ids(1)), ...
    '.', num2str(config.stack.case_ids(end)), '.tif'];
% Whether to scale images before displaying
...
config.reconstruction.name = '04062009';
...
config.linkage.include_proofread.proofread_name = 'anon';
...
config.proofreader.export.superpixel_map_filter_version = 'o4';
config.proofreader.import.proofread_name = 'test';
...
config.proofreader.import.Raveler.proofread_data_dir = ...
  '/groups/chklovskii/chklovskiilab/em_reconstruction/proofread_data/lamina.OTO.shinya.leginon.3500x.July2008/region.Lcartridge_elin_network.0.8.Lcartridge_elin744255586476_40/marta.2009.L1.linkage2_1/';

function config = seg_eval_medulla_Leginon_misc_LDA_prune_classify_test(config, case_ids, is_verbose, is_verbose_figures)
...
% ".tif".
config.stack.image_prefix = 'a.012.cropped%03d';
...
% () Superpixel method
config.superpixel_2_seg(1).superpixel_method = 'grayscale_AglMedianB';
% (b) Superpixel version to be used as basis
% % use superpixel segmentation parameters from previous step
% For grayscale-ladder .gs_l_T<f_threshold>_L<area_threshold>.<filter_version>_<mitochondria>
s = {};
...
config.superpixel_2_seg(1).f_thresholds = 0.00001;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
...
config.superpixel_2_seg(1).area_thresholds = 300;
% () Whether mitochondria are to be used to suppress false boundaries
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel_2_seg(1).filter_version = 'db_nwo500_LDA4_mf7_ps1\_0_neg';
% () whether to print messages
...
config.superpixel_2_seg(2).prune_classify.merge_criterion = 'region_max';
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel_2_seg(2).filter_version = 'db_nwo500_LDA4_mf7_ps1\_0_neg';
% () whether to print messages

function config = seg_eval_medulla_Leginon_misc_LDA_prune_classify(config, case_ids, is_verbose, is_verbose_figures)
...
% ".tif".
config.stack.image_prefix = 'a.012.cropped001.r%d';
% config.stack.image_prefix = 'a.%03d';  % 'a.%03d';  

config.stack.image_suffix = '.tif';
% (c) slice ids: If the images are a001.tif, ..., a010.tif then the
% case_ids are 1,2,...,10.
if(nargin<2 || isempty(case_ids))
  config.stack.case_ids = 1;
else
...
if(length(config.superpixel)==1)
  config.superpixel = repmat(config.superpixel, [4 1]);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
config.superpixel(1).watershed_filter_version = 'db_nwo500_LDA4_mf7_ps1\_0_neg';
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(1).filter_version = 'db_nwo500_LDA4_mf7_ps1\_0_neg';
...
for f = config.superpixel(1).save_f_thresholds %0.1
  s{end+1} = ...
    [ '.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
      ...
      '.db_nwo500_LDA4_mf7_ps1_0_neg_m0_d5_a40', ...
      '.db_nwo500_LDA4_mf7_ps1_0_neg']; %#ok<AGROW>
end
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(2).filter_version = 'db_nwo500_LDA4_mf7_ps1\_0_neg';
% () whether to print messages
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(3).filter_version = 'db_nwo500_LDA4_mf7_ps1\_0_neg';
% () whether to print messages
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% 5.4 Superpixel - prune-classify
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) Algorithm to be used - must match with the function called
config.superpixel(4).method = 'prune_classify';
% () Superpixel method
config.superpixel(4).superpixel_method = 'grayscale_ladder';
% (b) Superpixel version to be used as basis
% % use superpixel segmentation parameters from previous step
% For grayscale-ladder .gs_l_T<f_threshold>_L<area_threshold>.<filter_version>_<mitochondria>
s = {};
...
config.superpixel(4).superpixel_suffix = s; 
...
config.superpixel(4).prune_classify.merge_criterion = 'region_max';
% Parameters specific to the chosen merge criterion
% For region_max: the radius of the path. The patch's dimension is
% 2*patch_size+1
config.superpixel(4).prune_classify.merge_criterion_param.patch_size = 2;
% For region_max: the threshold on the maximum value observed within a
% patch
config.superpixel(4).prune_classify.merge_criterion_param.min_threshold = 0.2;
% Whether to save the segmentation overlaps as TIF files for viewing.
config.superpixel(4).save_segmentation_overlay = true;
% () Whether mitochondria are to be used to suppress false boundaries
config.superpixel(4).use_mitochondria = false;
% () The confidence threshold to be applied on the mitochondria
config.superpixel(4).mitochondria_confidence_threshold = 0.5; 
% () The thresholded mitochondria mask is eroded to ensure that correct
% boundaries are not obliterated
config.superpixel(4).mitochondria_erosion = 2; % 20
% * Minimum of a thresholded connected component for it to be considered
% mitochondria
config.superpixel(4).mitochondria_min_area = 40;
% () Whether to use vesicle detection to suppress false boundaries
config.superpixel(4).use_vesicle = false;
% () The threshold for the vesicle detection confidence
config.superpixel(4).vesicle_threshold = 0;
% () Radius of the vesicles. Boundaries within this distance from detected
% vesicle centers are obliterated.
config.superpixel(4).vesicle_dilation = 0;
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(4).filter_version = 'db_nwo500_LDA4_mf7_ps1\_0_neg';
% () whether to print messages
config.superpixel(4).is_verbose = is_verbose;
% () whether to display intermediate results
config.superpixel(4).is_verbose_figures = is_verbose_figures;

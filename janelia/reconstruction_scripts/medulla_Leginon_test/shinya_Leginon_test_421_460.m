function shinya_Leginon_test_421_460(module_id, case_ids)
% shinya_Leginon_test_421_460(module_id)
...
config.stack.image_structure = 'test_crop_421_460.xml';
% * slice ids: specify the z-plane numbers.
if(nargin>1)
  config.stack.case_ids = case_ids;
else
  config.stack.case_ids = 421:460;
end
...
config.reconstruction.name = '042809_421.460_1.5k.1.5k';
...
s = {};
for f = 0.06 %config.superpixel(1).save_f_thresholds
  s{end+1} = ...
...
config.linkage.train.save_suffix = '.04282009'; % better to change this into the date you do recon
...
config.linkage.apply.model_suffix = '.04282009'; % better to change this into the date you do recon
% (j) Verbose
...
config.proofreader.roi.xmin = 320; %4937; %3937;
config.proofreader.roi.ymin = 320; %3337; %2337;
config.proofreader.roi.xmax = 1819; %6436; %7436;
config.proofreader.roi.ymax = 1819; %4836; %5836;

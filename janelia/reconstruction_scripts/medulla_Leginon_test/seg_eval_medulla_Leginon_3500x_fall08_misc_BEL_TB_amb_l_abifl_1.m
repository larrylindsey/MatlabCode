function config = seg_eval_medulla_Leginon_3500x_fall08_misc_BEL_TB_amb_l_abifl_1(config, case_ids, is_verbose, is_verbose_figures)
...
config.superpixel(1).watershed_filter_version = 'BELc\_tbb5\_mf7_e2_d4';
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(1).filter_version = 'BELc\_tbb5\_mf7_e2_d4';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% 5.2 Superpixel - grayscale_ladder
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) Algorithm to be used - must match with the function called
...
for f = config.superpixel(1).save_f_thresholds %0.1
  s{end+1} = ...
    [ '.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
      '_b', num2str(0*(f>0),'%d'), ...
      '.BELc_tbb5_mf7_e2_d4_m0_d5_a40', ...
      '.BELc_tbb5_mf7_e2_d4']; %#ok<AGROW>
end
config.superpixel(2).superpixel_suffix = s; 
% () thresholds on the boundary fieldss
config.superpixel(2).f_thresholds = 0.00001;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(2).filter_version = 'BELc\_tbb5\_mf7_e2_d4';
% () whether to display intermediate results
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%
% % 5.3 Superpixel - grayscale ladder
% %%%%%%%%%%%%%%%%%%%%%%%%%%
% % (a) Algorithm to be used - must match with the function called
...
[ '.gs_l_sp_T1e-05_L300', ...
  ...
  '.BELc_tbb5_mf7_e2_d4'];
s = {};
for f = config.superpixel(1).save_f_thresholds(7) %config.superpixel(1).save_f_thresholds % 0.1
    s{end+1} = ...

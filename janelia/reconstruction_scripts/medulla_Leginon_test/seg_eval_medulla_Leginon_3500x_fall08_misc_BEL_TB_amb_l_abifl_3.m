function seg_eval_medulla_Leginon_3500x_fall08_misc_BEL_TB_amb_l_abifl_3(module_id)
...
% case_ids are 1,2,...,10.
config.stack.case_ids = 12:16; %7;%:11;
...
config.superpixel(1).f_threshold_seq = 0.01:0.01:0.07; %0.01:0.01:0.4;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(1).save_f_thresholds = 0.07; %[0.02:0.01:0.09, 0.1:0.05:0.4];
...
config.superpixel(1).watershed_filter_version = 'BELc\_tb5\_mf7_e2_d4';
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(1).filter_version = 'BELc\_tb5\_mf7_e2_d4';
% () whether to display intermediate results
config.superpixel(1).is_verbose = false;

%%%%%%%%%%%%%%%%%%%%%%%%%%
% 5.2 Superpixel - agglomerative median boundary
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) Algorithm to be used - must match with the function called
...
for f = 0.07
  s{end+1} = ...
    [ '.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
      '_b', num2str(0*(f>0),'%d'), ...
      '.BELc_tb5_mf7_e2_d4_m0_d5_a40', ...
      '.BELc_tb5_mf7_e2_d4']; %#ok<AGROW>
end
...
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(2).save_f_thresholds = 0.03; %[0.02, 0.1:0.05:0.6];
% Whether to save the segmentation overlaps as TIF files for viewing.
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(2).filter_version = 'BELc\_tb5\_mf7_e2_d4';
% () whether to display intermediate results
config.superpixel(2).is_verbose = false;
...
% For grayscale-ladder .gs_l_T<f_threshold>_L<area_threshold>.<filter_version>_<mitochondria>
s = {};
for f = 0.03; %config.superpixel(2).save_f_thresholds
  s{end+1} = ...
    [ '.gs_amdb_sp_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
      '_b', num2str(0*(f>0),'%d'), ...
      '.gs_amb_T0.07_0.06_b0', ...
      '.BELc_tb5_mf7_e2_d4_m0_d5_a40', ...
      '.BELc_tb5_mf7_e2_d4', ...
      '.BELc_tb5_mf7_e2_d4']; %#ok<AGROW>
end
...
% () thresholds on the boundary fieldss
config.superpixel(3).f_thresholds = 0.001:0.001:0.01;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(3).filter_version = 'BELc\_tb5\_mf7_e2_d4';
% () whether to display intermediate results
config.superpixel(3).is_verbose = false;
...
config.eval_segment_boundary.groundtruth_file = ...
  'region.fs.12.16.fs.12_13_14_15_16/analysis1/satoko.031009/proofread.seg.mat'; %'proofread.a.007.011.katerina.03192009.seg.mat';
...
[ '.gs_l_sp%s_L300', ...
  ...
  '.BELc_tb5_mf7_e2_d4'];
s = {};
for f = config.superpixel(3).save_f_thresholds % 0.1
    s{end+1} = ...
      [ '_T', num2str(f,'%g')]; %#ok<AGROW>
end
config.eval_segment_boundary.seg_suffix_id = s;

function shinya_Leginon_611_710_old_version(module_id, case_ids)
...
% Whether to run in stand alone mode
config.job.is_stand_alone = true;
...
config.region.case_ids = 161:310; % <<<<<<<<<<<<<<<<<<<<<<<<-----------
...
config.align.in_section_tile_pair_align.deformable_mesh.is_verbose = true;
% Whether to display intermediate results
config.align.in_section_tile_pair_align.deformable_mesh.is_verbose_figures = false;
% Filter images before feeding to SIFT feature point detector. By default
% no filtering or when defined empty.
config.align.in_section_tile_pair_align.deformable_mesh.filter_version = '';
% For deformable mesh, whether to read files from matlab_mat or tif_txt (as
% generated by binaries).
config.align.in_section_tile_pair_align.deformable_mesh.input_tform_format = 'tif_txt';
% Calculate adjacency matrix from trakEM2 tiling
...
config.linkage.train.save_suffix = '.08062009'; % better to change this into the date you do recon
...
config.linkage.apply.model_suffix = '.08062009'; % better to change this into the date you do recon
% (j) Verbose
...
config.proofreader.roi.xmin = 6228; %2504; %3864;
config.proofreader.roi.ymin = 6972; %3496; %3084;
config.proofreader.roi.xmax = 9227; %6003; %7363;
config.proofreader.roi.ymax = 9971; %6995; %6583;

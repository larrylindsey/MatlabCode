function config = medulla_phase4_161_1460_LDA_lp_shrink_ls(config, case_ids, is_verbose, is_verbose_figures)
% medulla_phase4_161_1460_LDA_lp_shrink_ls(module_id)
% phase4 reconstruction of medulla.HPF.Leginon.3500x.zhiyuan.fall2008.
% segmentation version 1.5 (LDA with recall almost the same as that used
% for pervious large scale reconstruction, i.e. 99.8%).
% 
% Shiv N. Vitaladevuni
...
config.region.case_ids = 161:170;
...
config.reconstruction.name = 'eval_lda1.5_ls_lp';
...
config.precompute.filtered_image.filter_version = 'db_nwo700_cbf2_LDA7_mf7_ps1\_0.25_abf_mito0\_d5\_e5\_a400_neg';
...
config.superpixel(1).mitochondria_confidence_threshold = 1; 
...
for f = config.superpixel(1).save_f_thresholds %0.1
  s{end+1} = ...
    [ '.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
      ...
      '.db_nwo700_cbf2_LDA7_mf7_ps1_0.25_abf_neg_m1_v2_f', ...
...
for f = config.superpixel(2).save_f_thresholds %0.1
  s{end+1} = ...
    [ '.gs_amdb_sp_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
      ...
      '.db_nwo700_cbf2_LDA7_mf7_ps1_0.25_abf_neg_f']; %#ok<AGROW>
end
...
%       '.gs_amb_T0.05_0.04_b0', ...
%       '.db_nwo700_cbf2_LDA7_mf7_ps1_0.25_abf_neg_m0_d5_a40_f', ...
%       '.db_nwo700_cbf2_LDA7_mf7_ps1_0.25_abf_neg', ...
%       '.db_nwo700_cbf2_LDA7_mf7_ps1_0.25_abf_neg_f', ...
%       '.db_nwo700_cbf2_LDA7_mf7_ps1_0.25_abf_neg_f'];
...
% () Superpixel method: %%%%% Deprecated method for parameters %%%%%
config.superpixel_2_seg(1).superpixel_method = '';
% (b) Superpixel version to be used as basis %%%%% Deprecated method for parameters %%%%%
% % use superpixel segmentation parameters from previous step
% For grayscale-ladder .gs_l_T<f_threshold>_L<area_threshold>.<filter_version>_<mitochondria>
...
%   maximum. Merge is one of these is below a threshold. Looks for large
%   gaps in the boundary confidence.
config.superpixel_2_seg(2).prune_classify.merge_criterion = 'merge_mito_segment';
% Parameters specific to the chosen merge criterion
% For region_max: the radius of the path. The patch's dimension is
% 2*patch_size+1
config.superpixel_2_seg(2).prune_classify.merge_criterion_param.patch_size = 2;
% % For region_max: the threshold on the maximum value observed within a
% % patch
% config.superpixel_2_seg(2).prune_classify.merge_criterion_param.min_threshold = 0.2;
% Whether to save the segmentation overlaps as TIF files for viewing.
...
config.linkage.feature.filter_version = 'db_nwo700_cbf2_LDA7_mf7_ps1_0.25_abf_neg';

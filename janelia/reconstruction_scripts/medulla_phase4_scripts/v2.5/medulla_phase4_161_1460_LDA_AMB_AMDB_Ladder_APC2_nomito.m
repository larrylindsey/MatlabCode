function config = medulla_phase4_161_1460_LDA_AMB_AMDB_Ladder_APC2_nomito(config, case_ids, is_verbose, is_verbose_figures)
...
config.reconstruction.name = 'phase4';
...
% () whether to include proofread superpixel map
config.superpixel(1).include_proofread.is_enabled = true;
% () suffix name of the proofread
config.superpixel(1).include_proofread.proofread_name = 'phase4';
...
for f = config.superpixel(1).save_f_thresholds %0.1
  s{end+1} = ...
    [ '.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
      ...
      '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg']; %#ok<AGROW>
end
config.superpixel(2).superpixel_suffix = s; 
% () thresholds on the boundary field
config.superpixel(2).f_threshold_seq = 0.01:0.01:0.35;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(2).save_f_thresholds = 0.35;
% Whether to save the segmentation overlaps as TIF files for viewing.
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(2).filter_version = 'db_nwo700_cbf3_LDA7_mf7_ps1\_0.25_abf_neg';
% () whether to print messages
...
for f = config.superpixel(2).save_f_thresholds
  for f2 = config.superpixel(1).save_f_thresholds
    s{end+1} = ...
      [ '.gs_amdb_sp_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
        '_b', num2str(5*(f>-1),'%d'), ...
        '.gs_amb_T', num2str(f2,'%g'), '_', num2str(f2-0.01,'%g'), ...
        '_b', num2str(25*(f2>0.65),'%d'), ...
        '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg_f', ...
        '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg', ...
        '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg_f']; %#ok<AGROW>
  end
end
config.superpixel(3).superpixel_suffix = s; 
% () thresholds on the boundary fieldss
config.superpixel(3).f_thresholds = 0.25;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(3).filter_version = 'db_nwo700_cbf3_LDA7_mf7_ps1\_0.25_abf_neg';
% () whether to print messages
...
% () suffix name of the proofread
config.superpixel_2_seg(1).include_proofread.proofread_name = 'phase4';
...
for f = config.superpixel(2).save_f_thresholds %0.1
  s{end+1} = ...
    [ '.apc_sp_region_max_3_0.35', ...
      ...
      '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg_m0_d5_a40', ...
      '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg', ...
      '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg', ...
      '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg', ...
      '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg']; %#ok<AGROW>
...
% merging.s
config.superpixel_2_seg(2).prune_classify.merge_criterion_param.threshold = 3;
% Whether to save the segmentation overlaps as TIF files for viewing.
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel_2_seg(3).filter_version = 'db_nwo700_cbf3_LDA7_mf7_ps1\_0.25_abf_neg';
% () whether to print messages
...
%     config.superpixel_choose.choice.seg_suffix, ...
%     '.db_nwo650_cbf3_LDA7_mf7_ps1.5_0.2_abf_neg'];
...
config.linkage.feature.filter_version = 'db_nwo700_cbf3_LDA7_mf7_ps1\_0.25_abf_neg';
...
config.linkage.modified_segment.suffix = '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg_m0.5_d2_a40_f';
...
config.eval_segment_boundary.seg_suffix_format = ...
    [ '.apc_sp_boundary_hist_boost_3', ...
      ...
      '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg', ...
      '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg', ...
      '.db_nwo700_cbf3_LDA7_mf7_ps1_0.25_abf_neg_m0.5_d2_a40'];
s = {};
for f = config.superpixel(2).save_f_thresholds

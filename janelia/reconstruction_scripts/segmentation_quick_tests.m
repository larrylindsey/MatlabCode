function segmentation_quick_tests(module_id)
% segmentation_quick_tests(module_id)
...
% ".tif".
config.stack.image_prefix = 'mean_boundary_agglo_seg_ex_%d';  
...
config.reconstruction.name = '08182008';
...
config.superpixel.length_threshold_seq = ...
  0*(config.superpixel.f_threshold_seq>-0.65);
...
config.superpixel.save_f_thresholds = ...
  config.superpixel.f_threshold_seq;
...
config.superpixel_2_seg(1).superpixel_suffix = '.gs_l_T0.5_L300.v_heq_mf3'; 
% (c) thresholds on the boundary field
config.superpixel_2_seg(1).f_threshold_seq = 0.5:0.01:0.85;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
% % use superpixel segmentation parameters from previous step
% For grayscale-ladder .gs_l_T<f_threshold>_L<area_threshold>.<filter_version>_<mitochondria m<threshold_d<amount of erosion>>
config.superpixel_2_seg(2).superpixel_suffix = '.gs_amb_sp_T0.77_0.76_b25.gs_l_T0.5_L300.v_heq_mf3.v_heq_mf2';
% '.gs_l_T0.47_L100.v_heq_mf3_e2_m0_d2_a500'; 
% (c) thresholds on the boundary field
config.superpixel_2_seg(2).f_threshold_seq = 0.73:0.01:0.9; 
% 0.48:0.01:0.9;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
config.superpixel_2_seg(3).superpixel_suffix = '.gs_amdb_sp_T0.79_0.78_b25.gs_amb_sp_T0.77_0.76_b25.gs_l_T0.5_L300.v_heq_mf3.v_heq_mf2.v_heq_mf3'; % '.BEL_l_T0.4_L200.5_b_v_mf5_d2'
% (c) thresholds on the boundary field
config.superpixel_2_seg(3).f_threshold_seq = 0.01:0.01:0.35; % :0.02:0.7; % 0.002:0.07;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise

function seg_eval_fly_larva_no_ua_10_BEL_thick_bndry_amb_l(module_id)
...
% detection).
config.superpixel(1).filter_version = 'BELtb.5_mf3';
...
for f = config.superpixel(1).save_f_thresholds
  s{end+1} = ['.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
    '_b', num2str(25*(f>0.65),'%d'), '.BELtb.5_mf3.BELtb.5_mf3'];
end
...
% detection).
config.superpixel(2).filter_version = 'BELtb.5_mf3';
% () whether to display intermediate results
...
% the format string.
config.eval_segment_boundary.seg_suffix_format = '.gs_l_sp_T0.2_L200%s.BELtb.5_mf3';
s = {};
for f = config.superpixel(1).save_f_thresholds
  s{end+1} = ['.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
    '_b', num2str(25*(f>0.65),'%d'), '.BELtb.5_mf3.BELtb.5_mf3'];

function seg_eval_fly_larva_no_ua_10_BEL_amb_l_abifl(module_id)
% seg_eval_fly_larva_no_ua_10_BEL_amb_l_abifl(module_id)
...
config.superpixel(1).save_f_thresholds = 0.04:0.04:0.4;
...
% detection).
config.superpixel(1).filter_version = 'BEL_mf7';
% Boundary version
config.superpixel(1).boundary_version = '5';
...
for f = config.superpixel(1).save_f_thresholds
  s{end+1} = ['.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
    '_b', num2str(25*(f>0.65),'%d'), '.BEL_mf7.BEL_mf7'];
end
...
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(2).save_f_thresholds = 0.005;
...
% detection).
config.superpixel(2).filter_version = 'BEL_mf7';
% Boundary version
config.superpixel(2).boundary_version = '5';
% () whether to display intermediate results
...
for i = 1:length(config.superpixel(2).superpixel_suffix)
  s{end+1} = ['.gs_l_sp_T0.005_L200', config.superpixel(2).superpixel_suffix{i}, ...
    '.BEL_mf7'];
end
config.superpixel(3).superpixel_suffix = s; 
% (c) thresholds on the boundary field
config.superpixel(3).f_threshold_seq = 0.01:0.01:0.08; % :0.02:0.7; % 0.002:0.07;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(3).save_f_thresholds = 0.01:0.01:0.08;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(3).filter_version = 'v_mf3';
% () whether to display intermediate results
...
% the format string.
config.eval_segment_boundary.seg_suffix_format = '.gs_abif_sp_T0.05_0.04_b2%s.v_mf3';

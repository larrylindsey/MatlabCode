function seg_eval_medulla_HPF_FS_0_2_gs_amb_l_abifl(module_id)
% seg_eval_medulla_HPF_FS_0_2_gs_l_abifl(module_id)
% Evaluate segmentation boundary on sec. 0 and 2 of stack
% medulla.GA5min_HPF_FS_0509.5000x.000_002.
% Algo: grayscale ladder + agglo. mean boundary + agglo. boundary vs.
% interior.
%
% module_id
...
config.superpixel(1).save_f_thresholds = 0.6:0.02:0.78;
...
for f = config.superpixel(1).save_f_thresholds
  s{end+1} = ['.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
    '_b', num2str(25*(f>0.65),'%d'), '.v_heq_mf3_m0_d2_a500.v_heq_mf3'];
end
config.superpixel(2).superpixel_suffix = s; 
% () thresholds on the boundary field
config.superpixel(2).f_thresholds = 0.3;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
...
for i = 1:length(config.superpixel(2).superpixel_suffix)
  s{end+1} = ['.gs_l_sp_T0.3_L200', config.superpixel(2).superpixel_suffix{i}, ...
    '.v_heq_mf5_e1'];
end

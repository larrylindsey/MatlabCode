function seg_eval_lamina_3500x_052308_3_5_gs_amb_l_abifl(module_id)
% seg_eval_lamina_3500x_052308_3_5_gs_amb_l_abifl(module_id)
% Evaluate segmentation on lamina.OTO.3500x.zhiyuan.May232008.3_5 stack
% Algorithm: grayscale agglo. mean boundary + grayscale ladder + agglo.
...
config.superpixel(1).save_f_thresholds = 0.56:0.02:0.76;
...
% detection).
config.superpixel(1).filter_version = 'v_heq_o1';
...
for f = config.superpixel(1).save_f_thresholds
  s{end+1} = ['.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
    '_b', num2str(25*(f>0.65),'%d'), '.v_heq_o1_m0_d5_a40.v_heq_o1'];
end
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(2).filter_version = 'v_heq_e1';
% () whether to display intermediate results
...
s = {};
for f = config.superpixel(1).save_f_thresholds
  s{end+1} = ['.gs_l_sp_T0.2_L100.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.01,'%g'), ...
    '_b', num2str(25*(f>0.65),'%d'), '.v_heq_o1_m0_d5_a40.v_heq_o1.v_heq_e1'];
end
...
config.eval_segment_boundary.seg_suffix_format = ...
  [ '.gs_abif_sp_T0.18_0.17_b2', ...
    '.gs_l_sp_T0.2_L100.gs_amb_T', ...
    '%s.v_heq_o1_m0_d5_a40.v_heq_o1', ...
    '.v_heq_e1', ...
    '.v0'];

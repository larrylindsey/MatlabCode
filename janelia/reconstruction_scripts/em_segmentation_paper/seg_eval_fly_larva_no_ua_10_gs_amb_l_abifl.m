function seg_eval_fly_larva_no_ua_10_gs_amb_l_abifl(module_id)
% seg_eval_fly_larva_no_ua_10_gs_amb_l_abifl(module_id)
...
% () thresholds on the boundary field
config.superpixel(2).f_thresholds = 0.2:0.05:0.6;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% 5.2 Superpixel - agglomerative boundary vs. interior values
...
for i = 1:length(config.superpixel(2).superpixel_suffix)
  s{end+1} = ['.gs_l_sp_T0.1_L200', config.superpixel(2).superpixel_suffix{i}, ...
    '.v_heq_mf5_e1'];
end
...
% the format string.
config.eval_segment_boundary.seg_suffix_format = '.gs_abif_sp_T0.18_0.17_b2%s.v_mf3';

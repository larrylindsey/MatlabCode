function seg_eval_lamina_3500x_052308_3_5_gs_nc_amb_l(module_id)
% seg_eval_lamina_3500x_052308_3_5_gs_nc_amb_l(module_id)
% Evaluate segmentation on lamina.OTO.3500x.zhiyuan.May232008.3_5 stack
% Algorithm: grayscale + normalized cuts + agglo. mean boundary + grayscale ladder
%
% module_id
...
config.superpixel(1).f_threshold_seq = 0.001:0.0001:0.06;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(1).save_f_thresholds = 0.01:0.004:0.06;
...
config.superpixel(1).watershed_filter_version = 'v_ncut_0';
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(1).filter_version = 'v_ncut_0';
...
for f = config.superpixel(1).save_f_thresholds
  s{end+1} = ['.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.0001,'%g'), ...
    '_b', num2str(0*(f>0.65),'%d'), '.v_ncut_0.v_ncut_0'];
end
config.superpixel(2).superpixel_suffix = s; 
% () thresholds on the boundary field
config.superpixel(2).f_thresholds = 0.003;
config.superpixel(2).save_f_thresholds = ...
  config.superpixel(2).f_thresholds;
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(2).filter_version = 'v_ncut_0';
% () whether to display intermediate results
...
s = {};
for f = config.superpixel(1).save_f_thresholds
  s{end+1} = [num2str(f,'%g'), '_', num2str(f-0.0001,'%g'), ...
    '_b', num2str(0*(f>0.65),'%d')];
end

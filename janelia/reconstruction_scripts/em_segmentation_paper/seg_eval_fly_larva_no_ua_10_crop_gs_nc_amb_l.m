function seg_eval_fly_larva_no_ua_10_crop_gs_nc_amb_l(module_id, case_ids)
% seg_eval_fly_larva_no_ua_10_crop_gs_nc_amb_l(module_id)
...
config.superpixel(1).save_f_thresholds = 0.1:0.025:0.4;
...
config.superpixel(1).watershed_filter_version = 'v_ncut_0_e4';
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(1).filter_version = 'v_ncut_0_e4';
...
for f = config.superpixel(1).save_f_thresholds
  s{end+1} = ['.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.001,'%g'), ...
    '_b', num2str(25*(f>0.65),'%d'), '.v_ncut_0_e4.v_ncut_0_e4'];
end
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(2).filter_version = 'v_ncut_0_e4';
% () whether to display intermediate results
config.superpixel(2).is_verbose = true;
...
% the format string.
config.eval_segment_boundary.seg_suffix_format = '.gs_l_sp_T0.0001_L200%s.v_ncut_0_e4';

function seg_eval_medulla_HPF_FS_0_2_gs_amb_l_noniter(module_id)
% seg_eval_medulla_HPF_FS_0_2_gs_l(module_id)
...
config.superpixel(1).f_threshold_seq = 0.52;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(1).save_f_thresholds = ...
  config.superpixel(1).f_threshold_seq; % 0.6:0.02:0.78;
...
config.superpixel(1).mitochondria_erosion = 2; % 20
...
config.superpixel(1).watershed_filter_version = 'v_heq_mf3';
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(1).filter_version = 'v_heq_mf3';
...
s = {};
for f = 0.52:0.02:0.58
  s{end+1} = ['.gs_amb_T', num2str(f,'%g'), '_', num2str(0,'%g'), ...
    '_b0', '.v_heq_mf3_m0_d2_a500.v_heq_mf3'];
end
config.superpixel(2).superpixel_suffix = s; 
% () thresholds on the boundary field
config.superpixel(2).f_thresholds = 0.1;
% () thresholds for which segmentations should be saved.
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(2).filter_version = 'v_heq_mf5_e1';
% () whether to display intermediate results
...
config.eval_segment_boundary.seg_suffix_format = ...
  [ '.gs_l_sp_T0.1_L200', ...
    '.gs_amb_%s_0_b0', ...
    '.v_heq_mf3_m0_d2_a500.v_heq_mf3', ...
    '.v_heq_mf5_e1'];
s = {};
for f = 0.52:0.02:0.58
  s{end+1} = ['T', num2str(f,'%g')];
end

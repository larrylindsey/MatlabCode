function seg_eval_medulla_HPF_FS_0_2_BEL_amb_l_abifl(module_id)
% seg_eval_medulla_HPF_FS_0_2_BEL_amb_l_abifl(module_id)
...
for f = config.superpixel(1).save_f_thresholds
  s{end+1} = ['.gs_amb_T', num2str(f,'%g'), '_', num2str(f-0.005,'%g'), ...
    '_b', num2str(25*(f>0.65),'%d'), '.BEL5_b_mf7.BEL5_b_mf7'];
end
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel(2).filter_version = 'BEL5\_b_mf7';
% Boundary version
...
s = {};
for f = config.superpixel(1).save_f_thresholds
  s{end+1} = ['.gs_l_sp_T0.005_L200.gs_amb_T', num2str(f,'%g'), '_', ...
    num2str(f-0.005,'%g'), '_b', num2str(25*(f>0.65),'%d'), ...
    '.BEL5_b_mf7.BEL5_b_mf7.BEL5_b_mf7'];
end
config.superpixel(3).superpixel_suffix = s; 
% (c) thresholds on the boundary field
config.superpixel(3).f_threshold_seq = 0.01:0.01:0.2;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel(3).save_f_thresholds = 0.02:0.02:0.2;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
config.eval_segment_boundary.seg_suffix_format = ...
  [ '.gs_abif_sp_T0.2_0.19_b20', ...
    '.gs_l_sp_T0.005_L200', ...
    '.gs_amb_T%s', ...
    '.BEL5_b_mf7.BEL5_b_mf7', ...
    '.BEL5_b_mf7', ...
    '.v0'];

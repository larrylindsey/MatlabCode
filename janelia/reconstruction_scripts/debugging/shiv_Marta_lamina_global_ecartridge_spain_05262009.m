function shiv_Marta_lamina_global_ecartridge_spain_05262009(module_id, case_ids)
...
config.stack.image_structure = 'global_ecartridge.xml'; % 'global_ecartridge_lacie.xml'
...
config.align.global_align.deformable_mesh.min_n_matches = 3;
...
config.align.global_align.display.save_as_stack_file_name = ...
  [ '~/temp/lamina_align_test_global_', num2str(config.stack.case_ids(1)), ...
...
% 
  exceptions{3,1} = [362,363]; exceptions{3,2} = '-NCP=300000 -MMA=200000 -DFT=0.21 -FFM=0.6 -FYL=0.48';
  exceptions{4,1} = [363,362]; exceptions{4,2} = '-NCP=300000 -MMA=200000 -DFT=0.21 -FFM=0.6 -FYL=0.48';
...
config.reconstruction.name = '06042009';
...
config.linkage.include_proofread.proofread_name = 'proofingrids';

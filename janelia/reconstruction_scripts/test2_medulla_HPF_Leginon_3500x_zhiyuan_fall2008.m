...
config.reconstruction.name = 'test2_medulla_HPF_SIFT';
...
config.superpixel.save_f_thresholds = 0.5:0.02:0.8; %0.68;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
% detection).
config.superpixel.filter_version = 'v_heq_e2'; %'v_heq';
...
config.superpixel_2_seg.superpixel_suffix = '.gs_amb_T0.7_0.69_b25.v_heq_o1_m0_d5_a40.v_heq'; 
% () thresholds on the boundary field
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel_2_seg.filter_version = 'v_heq_mf2'; %'v_heq_e2';
% () whether to display intermediate results

function seg_boundary_eval_gs_MeanBoundary_fly_larva_HPF_100708(module_id)
% seg_boundary_eval_gs_MeanBoundary_fly_larva_HPF_100708(module_id)
% Evaluate segmentation boundary on fly_larva.HPF.5s.rick.10072008. Not
% proofread yet (as of 10/07/2008), so no quantitative evaluation.
%
% module_id
...
% v4  10032008  Evaluation of segmentation for fly larva MW series (5th
%                 section) proofread by Satoko.
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) stack name
config.stack.name = 'fly_larva.HPF.5s.rick.10072008';   
% (b) image name:
...
% ".tif".
config.stack.image_prefix = '26278D5.Section%d.4800x.dm3';  
config.stack.image_suffix = '.tif';
% (c) slice ids: If the images are a001.tif, ..., a010.tif then the
% case_ids are 1,2,...,10.
config.stack.case_ids = 3;
% (d) ROI: Region Of Interest for the stack. If the entire slice image is
% to be processed then assign empty ([]). Otherwise get_roi(config);
config.stack.roi = [];
% config.stack.roi.xmin = 1;
% config.stack.roi.xmax = 4000;
% config.stack.roi.ymin = 2001;
% config.stack.roi.ymax = 4000;
...
config.superpixel.f_thresholds = 0.3; % :0.02:0.4;
% (c) minimum area of a segment - below this they merged with neighbors
config.superpixel.area_thresholds = 500;
...
% detection).
config.superpixel.filter_version = 'v_s7';
...
config.superpixel_2_seg(1).superpixel_suffix = '.gs_l_T0.3_L500.v_s7';
% (c) thresholds on the boundary field
config.superpixel_2_seg(1).f_threshold_seq = 0.3:0.01:0.9;
% () thresholds on the boundary field for which segmentations should be
% saved. Useful for reducing disk space.
config.superpixel_2_seg(1).save_f_thresholds = 0.55:0.01:0.62;
...
% detection).
config.superpixel_2_seg(1).filter_version = 'v_s7';
% () whether to display intermediate results
...
% % use superpixel segmentation parameters from previous step
% For grayscale-ladder .gs_l_T<f_threshold>_L<area_threshold>.<filter_version>_<mitochondria>
config.superpixel_2_seg(2).superpixel_suffix = '.gs_amb_sp_T0.58_0.57_b0.gs_l_T0.3_L500.v_s7.v_s7';
% (c) thresholds on the boundary field
config.superpixel_2_seg(2).f_threshold_seq = 0.01:0.01:0.1;
% () minimum boundary length (pixel units) for two segments to be merged
% should increase with increasing f_thresholds. E.g., 0 for f_thresholds
% <=0.65 and 20 otherwise
...
% merging.
config.superpixel_2_seg(2).max_area_threshold = 400000;
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel_2_seg(2).filter_version = 'v_s7';
% () whether to display intermediate results
...
% the format string.
config.eval_segment_boundary.seg_suffix_format = '.gs_amb_sp_T%s_b25.gs_l_T0.25_L300.v_heq_mf5.v_heq_mf5';
config.eval_segment_boundary.seg_suffix_id = {'0.68_0.67', '0.7_0.69', '0.72_0.71', '0.74_0.73'};

function linkage_customfeats_lamina_OTO_Jan222008_ministack(module_id)
%
% module_id
...
% 
% AG mod based on reconstruction_lamina_OTO_Jan222008_ministack.m
% Shiv N. Vitaladevuni
...
% case_ids are 6,...,10.
config.mitochondria.train.case_ids = 3:4;
...
config.mitochondria.feature.window_sizes = [7 10 15 20 25]; % [15 25 35];
...
% (f) Tree depth
config.mitochondria.model.tree_depth = 3; % 1
...
config.superpixel.mitochondria_confidence_threshold = 0.0; % 0.4
% (f) The thresholded mitochondria mask is eroded to ensure that correct
% boundaries are not obliterated
config.superpixel.mitochondria_erosion = 15; % 20
...
% detection).
config.superpixel.filter_version = 'v0';
...
config.superpixel_2_seg.superpixel_suffix = '.gs_l_T0.3_L600.v0_m0.5'; %v_aheq_m0.5'
% (c) thresholds on the boundary field
config.superpixel_2_seg.f_thresholds = 0.35;
...
config.superpixel_2_seg.use_mitochondria = true;
...
% detection).
config.superpixel_2_seg.filter_version = 'v_eq_mf5_e2';
...
config.linkage.train.manual_annotation_file = ...
    '~/data/lamina.OTO-1-k1.5000x.zhiyuan.Jan222008.0_2/manual_annotations/marta.05052008.seg.mat';
config.linkage.train.roi.ymin = 1;
config.linkage.train.roi.ymax = 1000;
% (b) Version name for the linkage model
config.linkage.train.save_suffix = '.custom7';
% (c) Type for feature to be used for linkage
config.linkage.feature.type = 'custom7';
...
config.linkage.model.type = 'logreg';
...
config.linkage.apply.model_suffix = '.custom7_gsladder';
...
config.linkage.apply.segmentation_suffix = ... 
  ['.gs_l_sp_T0.35_L600', config.superpixel_2_seg.superpixel_suffix, '.v0_m0.5']; %'.v_aheq_m0.5'


%%%%%%%%%%%%%%%%%%%%%%%%%%
% 8. Final reconstruction volume
%%%%%%%%%%%%%%%%%%%%%%%%%%
config.final_volume.link_thresholds = 0.5:0.02:1;

%%%%%%%%%%%%%%%%%%%%%%%%%%
% 9. Evaluate reconstruction volume
%%%%%%%%%%%%%%%%%%%%%%%%%%
config.evaluate_volume.groundtruth_file = 'marta.05052008.seg.mat';
config.evaluate_volume.roi.ymin = 1001;
config.evaluate_volume.roi.ymax = 2505;

function collect_seg_stats_groundtruth_lamina_3500x_052308_3_5(module_id)
% collect_seg_stats_groundtruth_lamina_3500x_052308_3_5(module_id)
% Collect statistics of intensity values along groundtruth segment
% boundaries. Used for training segmentation algorithms.
%
% module_id
...
% v3  08272008  modified for collecting statistics of intensity values
%               along boundaries for training segmentation algorithms.
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% 5/6. Segmentation using grayscale-ladder
%%%%%%%%%%%%%%%%%%%%%%%%%%
% (a) Algorithm to be used - must match with the function called
%config.segmentation_2D.method = 'grayscale_ladder';
% (b) thresholds on the boundary field
%config.segmentation_2D.f_thresholds = 0.3:0.05:0.7;
% (c) minimum area of a segment - below this they merged with neighbors
%config.segmentation_2D.area_thresholds = 150;
...
%config.segmentation_2D.mitochondria_erosion = 5;
...
% detection).
%config.segmentation_2D.filter_version = 'v_heq_e2';
...
config.superpixel_2_seg.superpixel_suffix = '.gs_amb_T0.1_0_b0.v_heq_o1_m0_d5_a40.v_heq'; 
% () thresholds on the boundary field
config.superpixel_2_seg.f_thresholds = 0.1;
...
% () The confidence threshold to be applied on the mitochondria
config.superpixel_2_seg.mitochondria_confidence_threshold = 0.5; 
% () The thresholded mitochondria mask is eroded to ensure that correct
% boundaries are not obliterated
config.superpixel_2_seg.mitochondria_erosion = 2; % 20
% * Minimum of a thresholded connected component for it to be considered
% mitochondria
config.superpixel_2_seg.mitochondria_min_area = 40;
% () Whether to use vesicle detection to suppress false boundaries
...
% () List of segments in the ground-truth that may be over-segmented without
...
% the format string.
config.eval_segment_boundary.seg_suffix_format = '.gs_l_sp_T0.1_L150.gs_amb_T%g_0.7_b25.v_heq_o1_m0_d5_a40.v_heq.v_heq_e2';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% () Ground truth segmentation to be used for training.
% Format similar to that for volume reconstruction evaluation.
% seg is a 3D matrix of body labels for the voxels.
config.train_segmentation.groundtruth_file = 'marta.06032008.seg.mat';
% () List of segments in the ground-truth that may be over-segmented without
% penalty. Use this for irrelevant structurs , e.g., glia.
config.train_segmentation.oversegment_ignore_groundtruth_label = 1045;
% () Type of boundary detection to be used. Addtional filters may be
% specified in train_segmentation.filter_version.
config.train_segmentation.boundary_method = 'grayscale';
% () Image filtering before performing segmentation (affects boundary
% detection).
config.train_segmentation.filter_version = 'v0';
% () Segmentation method to be analyzed
config.train_segmentation.segment_method = 'grayscale_AglMeanB';
% () Parameters for the method to be evaluated in the segmentation suffix
% There are two options:
% * Verbose suffix to provide a specific combination of parameters
% config.eval_segment_boundary.seg_suffix = '.gs_l_T0.56_L600.v_heq_mf10_m0';
...
% compatible printf format string and the list of ids to be printed with
% the format string.
config.train_segmentation.seg_suffix_format = ...
  '.gs_amb_T%s.v_heq_o1_m0_d5_a40.v_heq';
config.train_segmentation.seg_suffix_id =  {'0.1_0_b0','0.21_0.2_b0',...
  '0.31_0.3_b0','0.41_0.4_b0','0.51_0.5_b0','0.61_0.6_b0','0.71_0.7_b25'};
% () Maximum matching distance between ground-truth and automatic
% segmentation boundaries. Depends upon resolution. Keeping it very small
% results in zig-zag boundaries being penalized even though the overall
% segmentation is OK.
config.train_segmentation.max_match_dist = 15;

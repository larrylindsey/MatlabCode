function seg_training_data_AMB_seg_ex_5(module_id)
% seg_training_data_AMB_seg_ex_5(module_id)
% Generate training data for 2D segmentation -
% quick_tests/mean_boundary_agllo_seg_ex_5_(1,2)
% Agglo. median boundary
%
% module_id
...
% ".tif".
config.stack.image_prefix = 'mean_boundary_agglo_seg_ex_5_%d';  
...
config.reconstruction.name = 'seg_train_seg_ex_5';
...
config.superpixel.f_threshold_seq = 0.1:0.01:0.4;
...
config.superpixel_2_seg(1).superpixel_suffix = '.gs_amb_T0.4_0.39_b2.v_heq_o1.v0'; 
...
% () Image filtering before performing segmentation (affects boundary
% detection).
config.superpixel_2_seg(1).filter_version = 'v0';
% () whether to display intermediate results
...
config.linkage.apply.segmentation_suffix = ... 
  ['.gs_amb_sp_T0.73_0.72_b25', config.superpixel_2_seg(end).superpixel_suffix, '.v0'];
% (j) Verbose

function test_tile_pair_fold_dmesh_medulla_Leginon(module_id)
% test_tile_pair_fold_dmesh_medulla_Leginon(module_id)
% Test deformable mesh based tile pair alignment to handle folds on medulla stack
...
config.stack.image_structure = 'test_681_683_medulla_leginon_quick.xml';
% * slice ids: specify the z-plane numbers.
config.stack.case_ids = 681:683;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% 10. Alignment parameters
%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%
% 10.a For aligning tiles within a section
%%%
config.in_section_align.method = 'SIFT';
% (a) Calculate adjacency matrix from trakEM2 tiling
config.in_section_align.SIFT.trakEM2.getAdjacency=true;
% () Save SIFT landmarks, matches and transforms
config.in_section_align.SIFT.save=true;

%%%
% 10.b For aligning a pair of tiles in adjacent sections for linkage for
% stitching segmentations
%%%
% Method to be used for alignment, e.g., 'SIFT', 'deformable_mesh'
config.segmentation_align.method = 'SIFT';
% (a) Calculate adjacency matrix from trakEM2 tiling
config.segmentation_align.SIFT.trakEM2.getAdjacency=true;
% (b) Consistency threshold for RANSAC to consider aligning 
% a pair of images
config.segmentation_align.SIFT.RANSAC.consistency=0.3;
% () number of iterations of RANSAC. Transformation with minimum error
% among all iterations is chosen.
config.segmentation_align.SIFT.n_ransac_iter = 100;
...
config.segmentation_align.SIFT.downsampling=512;
% (d) Calculate SIFT landmarks and their descriptors
config.segmentation_align.SIFT.makeFrames=true;
% (e) Calculate matches between SIFT landmarks
config.segmentation_align.SIFT.makeMatches=true;
% (f) Calculate global affine transforms for each tile
config.segmentation_align.SIFT.makeTransforms=true;
% (g) Save SIFT landmarks, matches and transforms
config.segmentation_align.SIFT.save=true;
% () Whether to print intermediate messages
config.segmentation_align.SIFT.is_verbose = true;
...
config.segmentation_align.SIFT.min_n_matches = 50;
% () Whether to show the images inverted for easier assessment of alignment
config.segmentation_align.SIFT.is_inverted_display = true;

%%%
% 10.b For aligning a pair of tiles in adjacent sections for linkage
%%%
% Method to be used for alignment, e.g., 'SIFT', 'deformable_mesh'
config.linkage_align.method = 'deformable_mesh';

%%%
% 10.c For aligning tiles in adjacent sections for global alignment
%%%
config.inter_section_align.method = 'SIFT';
% (a) Calculate adjacency matrix from trakEM2 tiling
config.inter_section_align.SIFT.trakEM2.getAdjacency=true;
% () Save SIFT landmarks, matches and transforms
config.inter_section_align.SIFT.save=true;

%%%
% 10.d For aligning all tiles in global coordinates
%%%
config.global_align.method = 'SIFT';
% (a) Calculate adjacency matrix from trakEM2 tiling
config.global_align.SIFT.trakEM2.getAdjacency=true;
% (b) Consistency threshold for RANSAC to consider aligning 
% a pair of images
config.global_align.SIFT.RANSAC.consistency=0.3;
% () number of iterations of RANSAC. Transformation with minimum error
% among all iterations is chosen.
config.global_align.SIFT.n_ransac_iter = 100;
% () number of initial sample correspondences used for computing an initial
% estimate of model. Should be greater 2 for affine transformations.
% config.SIFT.n_initial_samples = 6;
% (c) Target downsampled image size for SIFT
config.global_align.SIFT.downsampling=512;
% (d) Calculate SIFT landmarks and their descriptors
config.global_align.SIFT.makeFrames=true;
% (e) Calculate matches between SIFT landmarks
config.global_align.SIFT.makeMatches=true;
% (f) Calculate global affine transforms for each tile
config.global_align.SIFT.makeTransforms=true;
% (g) Save SIFT landmarks, matches and transforms
config.global_align.SIFT.save=true;
% () Whether to print intermediate messages
config.global_align.SIFT.is_verbose = true;
% () Minimum number of matches between two tiles for their matches to be
% included in the section alignment. For pairs of tiles with marginal
% overlap, the matches are dominated by false correspondences. Therefore,
% the purity of union of matches is improved by eliminating small sets.
config.global_align.SIFT.min_n_matches = 50;
% () Whether to show the images inverted for easier assessment of alignment
config.global_align.SIFT.is_inverted_display = true;
...
config.reconstruction.name = 'test_dmesh_alignment';

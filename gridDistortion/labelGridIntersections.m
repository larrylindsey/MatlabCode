...
% 
% function [histLabel model] = getRegularPeaks(histogram)
% 
% % histogram = histogram / mean(histogram);
% 
% [sampleSpace block] = histogramToFenceStuff(histogram);
% 
% extraStruct.L = block;
% extraStruct.postProcess = @fenceSqDistPost;
% 
% count = 0;
% 
% err = inf;
% 
% while count < 5 && isinf(err) 
%     [set model err] = ransac(sampleSpace, @fenceModelDist,...
%         @fenceMetricFloatDist, 200, 5, 4, 500, extraStruct);
%     count = count + 1;
% end
% 
% if count >= 5
%     warning('labelGridIntersections:getRegularPeaks',...
%         'Could not find suitable histogram model. Using default labelling.');
%     histLabel = bwlabel(histogram > 0);
%     return;
% end
% 
% labels = block(:,1);
% 
% histLabel = zeros(size(labels));
% 
% for i_m = mod(model(1), model(2)):model(2):numel(labels)
%     lnum = labels(round(i_m));
%     if lnum > 0
%         lselect = logical(labels == lnum);
%         histLabel(lselect) = lnum;
%     end
% end
% 
% end
